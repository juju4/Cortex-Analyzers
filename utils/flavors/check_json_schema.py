...
def fixJsonFlavorFile(jsonfile: dict) -> dict:
    service_logo = {"path": "", "caption": "logo"}

    screenshots = [{"path": "", "caption": ""}]
    for I in [
...
def run():
    ...
    parser.add_argument(
        "-r",
        "--report",
        help="Generate report for all JSON flavors of all Analyzers and responders",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-x",
        "--fix",
        help="Adding new information required for documentation",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-f", "--file", help="Validate JSON of the Flavor definition file"
    )
    parser.add_argument(
        "-s",
        "--schema",
        help="JSON Schema of a flavor",
        default="utils/flavors/flavor_schema.json",
    )
            ...
            if args.file:
                ...
                try:
                    if os.path.isfile(filepath) and filepath.endswith(".json"):
                        ...
                        print(
                            "Error: Check this is a file, json formatted, and it has .json extention\n {}".format(
                                filepath
                            )
                        )
