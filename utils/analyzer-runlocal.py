...
class AnalyzerRunlocal:
    ...
    def __parse_args__(self):
        ...
        if (
            not args.a
            or not os.path.isfile(args.a)
            or not args.j
            or not os.path.isfile(args.j)
        ):
            argparse.print_help()
            print()
            exit(1)
    ...
    def check_job_definition(self, job_definition):
        ...
        if "dataType" not in job_definition:
            raise AnalyzerRunlocalException(
                'job_definition is missing a "dataType" dict key value'
            )
        if "data" not in job_definition:
            raise AnalyzerRunlocalException(
                'job_definition is missing a "data" dict key value'
            )
    ...
    def shell_command(self, command_line, timeout_seconds=300, encoding="utf8"):
        ...
        return (
            (None if stdout is None else stdout.decode(encoding)),
