...
class AWX(Responder):
    ...
    def run(self):
        ...
        payload = {"extra_vars": json.dumps(self.observable_all)}
        # Start the job
        job_start_endpoint = (
            self.url + "/api/v2/job_templates/" + self.workflow_id + "/launch/"
        )
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.error(
                    f"AWX job execution returned unexpected status {response.status_code}: {error_msg}"
                )
            ...
            try:
                ...
                error_details = response.json().get(
