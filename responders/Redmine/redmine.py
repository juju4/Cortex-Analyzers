...
class Redmine(Responder):
    def __init__(self):
        ...
        self.project_field = self.get_param(
            "config.project_field", None, "Missing custom field for Redmine project"
        )
        self.tracker_field = self.get_param(
            "config.tracker_field", None, "Missing custom field for Redmine tracker"
        )
        self.assignee_field = self.get_param(
            "config.assignee_field", None, "Missing custom field for Redmine assignee"
        )
        ...
        if self.reference_field:
            ops.append(
                self.build_operation(
                    "AddCustomFields",
                    name=self.reference_field,
                    tpe="string",
                    value="{}#{}".format(
                        self.instance_name, raw["issue"]["issue"]["id"]
                    ),
                )
            )
    ...
    def extract_case_data(self, data_root="data"):
        issue_data = {}
        issue_data["title"] = self.get_param(
            "{}.title".format(data_root), None, "Case title is missing"
        )
        issue_data["description"] = self.get_param(
            "{}.description".format(data_root), None, "Case description is missing"
        )
        issue_data["severity"] = self.get_param("{}.severity".format(data_root))
        if self.project_field:
            issue_data["project"] = self.get_param(
                "{}.customFields.{}.string".format(data_root, self.project_field),
                None,
                "Project not defined in case",
            )
        if self.tracker_field:
            issue_data["tracker"] = self.get_param(
                "{}.customFields.{}.string".format(data_root, self.tracker_field),
                None,
                "Tracker not defined in case",
            )
        if self.assignee_field:
            issue_data["assignee"] = self.get_param(
                "{}.customFields.{}.string".format(data_root, self.assignee_field), None
            )
