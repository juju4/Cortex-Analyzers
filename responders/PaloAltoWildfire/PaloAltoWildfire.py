...
class PaloAltoWildfire(Responder):
    def __init__(self):
        ...
        self.observable_description = self.get_param(
            "data.message", None, "Description is empty"
        )
    ...
    def run(self):
        ...
        try:
            ...
            if self.observable_type in supported_observables:
                ...
                headers = {"User-Agent": "PaloAltoWildfire-Cortex-Responder"}
                payload = {
                    "apikey": (None, self.api_key),
                    "link": (None, observable),
                }
                response = requests.post(
                    self.wildfire_url, files=payload, headers=headers
                )
                if response.status_code == 200:
                    self.report(
                        {
                            "message": "Observable sent to Wildfire. Message: {}".format(
                                response.text
                            )
                        }
                    )
                    ...
                    self.error(
                        {
                            "message": "Failed authentication. Check API-Key. Message: {}".format(
                                response.text
                            )
                        }
                    )
                    ...
                    self.error(
                        "Failed to submit request. Error code: {}. Error message: {}".format(
