...
class KnowBe4(Responder):
    def __init__(self):
        ...
        self.api_url = self.get_param("config.api_url", None, "Base URL Missing")
        self.hive_url = self.get_param("config.hive_url", None, "Hive URL Missing")
        ...
        if self.get_param("data.dataType") == "mail":
            ...
            if self.required_tag in tags or self.required_tag is None:
                ...
                description = "TheHive Case: {}\n Description: {}\n URL: {}".format(
                    self.get_param("data.case.title"),
                    self.get_param("data.case.description"),
                    thehive_case,
                )
                ...
                self.error(
                    "Email address not tagged with the required tag. "
                    + self.required_tag
                )
        else:
