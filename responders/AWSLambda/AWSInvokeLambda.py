...
class AWSLambda(Responder):
    def __init__(self):
        ...
        self.aws_access_key_id = self.get_param(
            "config.aws_access_key_id", None, "AWS Access Key ID missing"
        )
        self.aws_secret_access_key = self.get_param(
            "config.aws_secret_access_key", None, "AWS Secret Access Key missing"
        )
        self.aws_region = self.get_param(
            "config.aws_region", None, "AWS Region missing"
        )
        self.lambda_function_name = self.get_param(
            "config.lambda_function_name", None, "Lambda Function Name missing"
        )
        self.invocation_type = self.get_param(
            "config.invocation_type", None, "RequestResponse"
        )
