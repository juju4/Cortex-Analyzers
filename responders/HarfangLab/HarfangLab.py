    ...
    if t and len(headers) > 0:
        ...
        if headerTransform is None:  # noqa
            ...
            def headerTransform(s):
                return string_escape(s.title())  # noqa
        ...
        for entry in t:
            ...
            vals = [
                string_escape(
                    str(entry_copy.get(h, "") if entry_copy.get(h) is not None else "")
                )
                for h in headers
            ]
...
def url_to_clickable_markdown(data, url_keys):
    ...
    if isinstance(data, list):
        ...
        data = {
            key: get_clickable_url(value, url_keys.get(key, None))
            if key in url_keys
...
class HarfangLab(Responder):
    ...
    def __generate_link_for_binary(self, v):
        ...
        try:
            token = self.hlSession.post(url=url, data={"is_expirable": True}).json()
    ...
    def __generate_link_for_artifact(self, v):
        ...
        try:
            ...
            if "api_token" in token:
                api_token = token["api_token"]
            link = (
                f"{self.apiURL}/api/data/investigation/artefact/Artefact/{v}/download/"
            )
            if api_token:
                link += f"?hl_expiring_key={api_token}"
    ...
    def __get_kill_process_api_endpoint(self):
        ...
        if not self.processUUID:
            self.error(
                f'Missing process unique identifier field: "hfl/process/process_unique_id'
            )
            return None
    ...
    JOBS = {
        "getProcesses": {
            ...
            "fields": [
                ...
                {
                    "name": "signed",
                    "path": "binaryinfo.binaryinfo.signed",
                    "default": False,
                },
                {"name": "md5", "path": "binaryinfo.binaryinfo.md5", "default": None},
                {"name": "sha1", "path": "binaryinfo.binaryinfo.sha1", "default": None},
                {
                    "name": "sha256",
                    "path": "binaryinfo.binaryinfo.sha256",
                    "default": None,
                },
            ],
        },
        ...
        "getPipes": {
            ...
            "action": "getPipeList",
            "ordering": "name",
            "fields": [{"name": "name", "path": "name", "default": None}],
        },
        "getDrivers": {
            "request_api_endpoint": "/api/data/Job/",
            "status_api_endpoint": "/api/data/Job/",
        ...
        "getPrefetches": {
            ...
            "fields": [
                {"name": "executable name", "path": "executable_name", "default": None},
                {
                    "name": "last executed",
                    "path": "last_executed",
                    "default": None,
                    "transform": __get_first_element,
                },
            ],
        },
        "getScheduledTasks": {
        ...
        "getStartupFiles": {
            ...
            "ordering": "filename",
            "fields": [
                {"name": "startup file name", "path": "filename", "default": None},
                {
            ...
            "fields": [
                {
                    ...
                    "path": "filtertoconsumertype",
                    "default": None,
                },
                {
                    "name": "event filter name",
                    "path": "eventfiltername",
                    "default": None,
                },
                {
                    "name": "event consumer name",
                    "path": "eventconsumername",
                    "default": None,
                },
        ...
        "getNetworkShares": {
            ...
            "fields": [
                ...
                {"name": "share type val", "path": "sharetypeval", "default": None},
        ...
        "getSessions": {
            ...
            "fields": [
                {"name": "logon id", "path": "logonid", "default": None},
                {
                    ...
                    "path": "authenticationpackage",
                    "default": None,
                },
                {"name": "logon type", "path": "logontype", "default": None},
                {"name": "logon type str", "path": "logontypestr", "default": None},
                {
        ...
        "getArtifactMFT": {
            ...
            "fields": [
                ...
                {"name": "size", "path": "size", "default": None},
                {
                    ...
                    "path": "id",
                    "default": None,
                    "transform": __generate_link_for_artifact,
                    "is_url": True,
                    "link_text": "Download",
                },
            ],
    ...
    TELEMETRY_SEARCHES = {
        "searchHash": {
            ...
            "inputs": [
                {
                    "name": "hash",
                    "filter": "hash",
                    "transform": __get_hash_parameter,
                    "mandatory": True,
                },
            ...
            "fields": [
                ...
                {"name": "integrity level", "path": "integrity_level", "default": None},
                {"name": "parent image", "path": "parent_image", "default": None},
                {
                    "name": "parent command line",
                    "path": "parent_commandline",
                    "default": None,
                },
                {"name": "username", "path": "username", "default": None},
        ...
        "getBinary": {
            ...
            "fields": [
                ...
                {"name": "sha256", "path": "hashes.sha256", "default": None},
                {
                    ...
                    "path": "hashes.sha256",
                    "default": None,
                    "transform": __generate_link_for_binary,
                    "is_url": True,
                    "link_text": "Download",
                },
            ],
        ...
        "searchSourceIP": {
            ...
            "fields": [
                ...
                {"name": "destination address", "path": "daddr", "default": None},
            ...
            "inputs": [
                {
                    "name": "ip",
                    "filter": "daddr",
                    "transform": __get_destination_ip,
                    "mandatory": True,
                },
                {"name": "limit", "filter": "limit"},
            ],
        ...
        "searchDriverByFileName": {
            ...
            "inputs": [{"name": "filename", "filter": "imagename", "manatory": True}],
            "fields": [
    ...
    def __init__(self):
        ...
        self.apiURL = self.get_param("config.apiURL", None, "API URL is missing!")
        self.apiKey = self.get_param("config.apiKey", None, "API Key is missing!")
        ...
        if self.data_type == "thehive:case_task":
            self.agentId = (
                self.get_param(
                    "data.case.customFields", {}, "Case custom fields are missing"
                )
                .get("hfl/agent/agentid", {})
                .get("string", None)
            )
            self.agentHostname = (
                self.get_param(
                ...
                .get("hfl/agent/hostname", {})
                .get("string", None)
            )
        elif self.data_type == "thehive:case" or self.data_type == "thehive:alert":
            self.agentId = (
                self.get_param(
                    "data.customFields", {}, "Case custom fields are missing"
                )
                .get("hfl/agent/agentid", {})
                .get("string", None)
            )
            ...
            self.processUUID = (
                ...
                .get("hfl/process/process_unique_id", {})
                .get("string", None)
            )
        ...
        self.hlSession.headers.update({"Authorization": f"Token {self.apiKey}"})
    ...
    def run(self):
        ...
        def run_job(job_name):
            ...
            if not self.agentId:
                self.error(
                    'Not agent identifier found. It must be in a case or alert custom field "hfl/agent/agentid".'
                )
                return
            ...
            try:
                ...
                if isinstance(data, list):
                    if len(data) == 0:
                        self.error("Failed to start job (wrong agent identifier ?)")
                        return
                    job_id = data[0]["id"]
                elif isinstance(data, dict):
                    job_id = data["job_id"]
            ...
            # Get Job results
            url = (
                f"{self.apiURL}{job['result_api_endpoint']}?limit=10000&job_id={job_id}"
            )
        ...
        if self.service == "isolateEndpoint":
            ...
            if not self.agentId:
                self.error("No agent Id found in the case or alert description")
                return
        ...
        if self.service == "isolateEndpoint":
            ...
            return [
                self.build_operation(
                    "CreateTask",
                    title=HarfangLab.JOBS.get(self.service).get("task_title", ""),
                    description=raw.get("markdown"),
                )
            ]
        elif self.service in HarfangLab.TELEMETRY_SEARCHES:
            return [
                self.build_operation(
                    "CreateTask",
                    title=HarfangLab.TELEMETRY_SEARCHES.get(self.service).get(
                        "task_title", ""
                    ),
                    description=raw.get("markdown"),
                )
