    ...
    def run(self):
        ...
        if self.service == "acquire":
            ...
            if response.status_code == requests.codes.ok:
                self.report(
                    {"message": f"Acquisition task has been started in {self.hostname}"}
                )
            else:
                self.error(
        ...
        if self.service == "isolate":
            ...
            if response.status_code == requests.codes.ok:
                self.report(
                    {"message": f"Isolation task has been started in {self.hostname}"}
                )
            else:
                self.error(
    ...
    def operations(self, raw):
        return [
            self.build_operation(
                "AddTagToArtifact", tag=f"BinalyzeAIR:{self.service}d the endpoint."
            )
        ]
