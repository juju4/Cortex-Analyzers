...
class Velociraptor(Responder):
    def __init__(self):
        ...
        self.configpath = self.get_param(
            "config.velociraptor_client_config", None, "File path missing!"
        )
        self.config = yaml.load(open(self.configpath).read(), Loader=yaml.FullLoader)
        self.artifact = self.get_param(
            "config.velociraptor_artifact", None, "Artifact missing!"
        )
        self.upload_flow_results = self.get_param(
            "config.upload_flow_results", None, "Upload decision missing!"
        )
    ...
    def run(self):
        ...
        with grpc.secure_channel(
            self.config["api_connection_string"], creds, options
        ) as channel:
            stub = api_pb2_grpc.APIStub(channel)

            if self.observable_type == "ip":
                client_query = (
                    "select client_id from clients() where last_ip =~ '"
                    + self.observable
                    + "'"
                )
            elif re.search(r"fqdn|other", self.observable_type):
                client_query = (
                    "select client_id from clients(search='host:"
                    + self.observable
                    + "')"
                )
            else:
            ...
            for response in stub.Query(request):
                try:
                    ...
                    if self.upload_flow_results is True:
                        ...
                        vfs_query = (
                            "SELECT create_flow_download(client_id='"
                            + str(flow["request"]["client_id"])
                            + "', flow_id='"
                            + str(flow["flow_id"])
                            + "', wait='true') as VFSPath from scope()"
                        )
                        ...
                        for vfs_response in stub.Query(vfs_request):
                            try:
                                vfs_result = json.loads(vfs_response.Response)[0][
                                    "VFSPath"
                                ]
                            except:
                                pass
                        ...
                        api = TheHiveApi(
                            self.thehive_url, self.thehive_apikey, cert=False
                        )

                        description = (
                            "Velociraptor flow for artifact"
                            + self.artifact
                            + "for client "
                            + client_id
                            + " via flow "
