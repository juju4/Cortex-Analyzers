...
class PushArtifactToProject(IlluminateServiceFile):
    ...
    def run(self, data):
        ...
        request = ProjectsRequest(
            username=self._params["username"], api_key=self._params["api_key"]
        )
        request.set_context(
            "thehive", "riq-responder", VERSION, self.__class__.__name__
        )
        ...
        if artifact_tag is not None:
            ...
            tags = None
        self._report["riq_artifact"] = request.upsert_artifact(
            project_guid, data["data"], tags=tags
        )
...
if __name__ == "__main__":
    ...
    cmdgroup.add_argument(
        "--test", dest="cmd", action="store_const", const="test", help="Test a service"
    )
    parser.add_argument(
        "--responder", choices=SERVICES.keys(), help="Responder to test."
    )
    parser.add_argument("--input", help="Input value of the IOC to test.")
    parser.add_argument(
        "--type",
        choices=["fqdn", "domain", "ip"],
    ...
    parser.add_argument(
        "--apikey", help="API key; will be read from passivetotal lib if not provided"
    )
    parser.add_argument(
        "--days-back", default=180, type=int, help="Number of days back to search."
    )
