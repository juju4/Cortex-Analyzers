...
class RT4(Responder):
    ...
    def run(self):
        ...
        if self.instance_type == "case":
            ...
            headers = {
                ...
                "Authorization": "Bearer {}".format(thehive_token),
            }
            thehive_api_url_case_search = "{}/api/case/artifact/_search".format(
                thehive_url
            )
            r = requests.post(
                thehive_api_url_case_search, data=json.dumps(payload), headers=headers
            )
        ...
        for observable in observable_list:
            ...
            or data that does not correspond to an RT field. Observable info: {}""".format(
                    observable
                )
                ...
                ticket_url = self.server + "/Ticket/Display.html?id={}".format(
                    new_ticket
                )
                self.return_info["successes"].append(
                    {"id": new_ticket, "msg": msg, "ticket_url": ticket_url}
                )
    ...
    def operations(self, raw):
        ...
        if self.instance_type == "case_artifact":
            return [
                self.build_operation("AddTagToArtifact", tag="rt4:submitted"),
                self.build_operation(
                    "AddCustomFields",
                    name=self.thehive_cf_rtticket,
            ...
            return [
                self.build_operation("AddTagToAlert", tag="rt4:submitted"),
                self.build_operation(
                    "AddCustomFields",
                    name=self.thehive_cf_rtticket,
                    value=created_tickets,
            ...
            return [
                self.build_operation("AddTagToCase", tag="rt4:submitted"),
                self.build_operation(
                    "AddCustomFields",
                    name=self.thehive_cf_rtticket,
                    value=created_tickets,
    ...
    def process_observables(self, data):
        ...
        for i in data:
            ...
            obs_config_tmp = {"indicator_list": [i["data"]]}
    ...
    def process_tags(self, tags):
        ...
        for tag in tags:
            ...
            if tag.lower().startswith("rt4_set_") and not tag.lower().startswith(
                "rt4_set_requestor"
            ):
                rt_setting_name, rt_setting_value = tag.split("rt4_set_")[1].split(
                    ":", 1
                )
                ...
                # cover cc, bcc, or admincc tags
                    ...
                    try:
                        ...
                        self.error(
                            "One of the rt4_set_ tags was not recognized: {}".format(e)
                        )
                    processed_tags.update({rt_setting_name: rt_setting_value})

            elif tag.lower().startswith("contact:") or tag.lower().startswith(
                "rt4_set_requestor"
            ):
    ...
    def create_rt_ticket(self, observable):
        ...
        if "Requestor" in observable:
            obs_config.update(
                weight="observable", **{"Requestor": observable["Requestor"]}
            )
        ...
        for key, value in obs_config.items():
            if (
                obs_config[key] is not None
                and key != "indicator_list"
                and key != "template"
            ):
                rt_ticket[key] = value
    ...
    def _dict_compare(self, d1, d2):
        ...
        try:
            ...
            self.error(
                """Could not get keys from dicts for comparison. dict1:
