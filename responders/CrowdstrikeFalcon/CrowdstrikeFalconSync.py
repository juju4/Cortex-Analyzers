...
class CrowdstrikeFalconSync(Responder):
    def __init__(self):
        ...
        self.custom_field_name_alert_id = self.get_param(
            "config.custom_field_name_alert_id"
        )
        self.custom_field_name_incident_id = self.get_param(
            "config.custom_field_name_incident_id"
        )
    ...
    def run(self):
        if self.service == "sync":
            ...
            current_stage = self.get_param(
                "data.stage", None, "Can't get case or alert stage"
            )
            detection_id = self.get_param(
                f"data.customFieldValues.{self.custom_field_name_alert_id}", None
            )
            incident_id = self.get_param(
                f"data.customFieldValues.{self.custom_field_name_incident_id}", None
            )

            if not detection_id and not incident_id:
                self.error(
                    "No detection ID or incident ID found in the case custom fields."
                )
            ...
            if detection_id:
                ...
                alert_body = {
                    ...
                    "action_parameters": [
                        {"name": "update_status", "value": cs_status_alert}
                    ],
                }
            ...
            if incident_id:
                ...
                incident_body = {
                    "ids": incident_id,
                    "action_parameters": [
                        {"name": "update_status", "value": cs_status_incident}
                    ],
                }

                incident_response = incident_client.perform_incident_action(
                    body=incident_body
                )
            ...
            if detection_id:
                if 200 <= alert_status_code < 300:
                    messages.append(
                        f"Successfully updated CrowdStrike alert(s): {detection_id} to status '{cs_status_alert}'."
                    )
                else:
                    messages.append(
                        f"Failed to update alert(s): {alert_response.get('body', 'No additional error information available')}."
                    )

            if incident_id:
                if 200 <= incident_status_code < 300:
                    messages.append(
                        f"Successfully updated CrowdStrike incident(s): {incident_id} to status '{cs_status_incident}'."
                    )
                else:
                    messages.append(
                        f"Failed to update incident(s): {incident_response.get('body', 'No additional error information available')}."
                    )
