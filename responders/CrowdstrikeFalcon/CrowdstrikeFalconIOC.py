    ...
    def run(self):
        if self.service == "addIOC":
            ...
            if 200 <= status_code < 300:
                self.report(
                    {
                        "message": f"{ioc_value} has been added to IoC Management as a {ioc_type} with action : {action}"
                    }
                )
            else:
                self.error(
                    f"An error occurred: {str(status_code)} -- {response.get('body', 'No additional error information available')}"
                )
            ...
            if 200 <= status_code < 300:
                ...
                if resources:
                    ...
                    if 200 <= delete_status_code < 300:
                        message = (
                            f"IOC {ioc_value} deleted successfully. IOC ID : {ioc_id}"
                        )
                    else:
