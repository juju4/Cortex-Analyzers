...
class CrowdstrikeFalconHosts(Responder):
    ...
    def run(self):
        ...
        if 200 <= response["status_code"] < 300:
            ...
            return self.error(
                f"Error on getting device ID : {response['body']['errors']}"
            )

        if hosts_ids:
            action_response = hosts.perform_action(
                action_name=self.service, ids=hosts_ids
            )
            if 200 <= action_response["status_code"] < 300:
                return self.report(
                    {
                        "message": f"Operation {self.service} has been performed on {hostname} successfully: {action_response['body']['resources']}"
                    }
                )
            else:
                return self.error(
                    f"Error on performing operation {self.service} on {hostname}: {action_response['body']['errors']}"
                )
        else:
        ...
        if self.service in ["contain", "hide_host", "detection_suppress"]:
            operations_list.append(
                self.build_operation(
                    "AddTagToCase",
                    tag=f"containment:{self.get_param('config.service', None)}={self.get_param('data.data', None)}",
                )
            )
