...
class Gatewatcher_CTI_Identity(Responder):
    def __init__(self):
        ...
        self.lis_api_key = self.get_param(
            "config.LISApiKey", None, "Gatewatcher CTI API KEY is required"
        )
        self.lis_base_url = "https://api.client.lastinfosec.com/v2/"
        self.theHive_api_key = self.get_param(
            "config.theHiveApiKey", None, "theHive API KEY is required"
        )
        self.theHive_fqdn = self.get_param(
            "config.theHiveFQDN", None, "theHive fqdn is missing"
        )
        self.headers = {
    ...
    def lis_get_by_domain(self, domain: str) -> list[dict[str, any]]:
        ...
        added_atfer = (
            subtract_minutes_from_now(minutes=self.datetime)
    ...
    def thehive_get_alert(self, field: str, value: str) -> list[dict[str, any]]:
        response = requests.post(
            ...
            json={
                "query": [
                    {"_name": "listAlert"},
                    {"_name": "filter", "_and": [{"_field": field, "_value": value}]},
                ]
            },
        )
    ...
    def thehive_generate_alert(self, data) -> Union[str, None]:
        ...
        alerts = self.thehive_get_alert(
            field="sourceRef", value=f"Gatewatcher CTI Identity - {email}"
        )
        if len(alerts) > 0:
            alert = alerts[0]
            ...
            if alert.get("_updatedAt") is not None:
                if alert.get("_updatedAt") < convert_datetime_to_timestamp(
    ...
    def thehive_import_alert_observables_to_case(
        ...
        if response.status_code != 200 or response.status_code == 400:
            ...
            if (
                ...
                and "Alert is already imported" in response.text
            ):
                self.thehive_unlink_alert(case_id=case_id, alert_id=alert_id)
                self.thehive_import_alert_observables_to_case(
    ...
    def run(self):
        ...
        for email in leaked_emails.get("message"):
            if (alert_id := self.thehive_generate_alert(email)) is not None:
                observable_count += 1
                self.thehive_import_alert_observables_to_case(
                    alert_id=alert_id, case_id=self.case_id
                )
