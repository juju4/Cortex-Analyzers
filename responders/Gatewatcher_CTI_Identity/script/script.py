...
def cortex_enable_responder(conf: dict[str, any]):
    response = requests.post(
        url=f"{cortex_fqdn}/api/organization/responder/{responder_name}",
        headers=cortex_header,
        json=conf,
    )
...
def cortex_desable_responder(responder_id: str):
    response = requests.delete(
        url=f"{cortex_fqdn}/api/organization/responder/{responder_id}",
        headers=cortex_header,
    )
...
def cortex_update_responder_conf(conf: dict[str, any], responder_id: str):
    response = requests.patch(
        url=f"{cortex_fqdn}/api/responder/{responder_id}",
        headers=cortex_header,
        json=conf,
    )
    if response.status_code != 200:
...
def thehive_delete_case(case_id: str):
    response = requests.delete(
        url=f"{theHive_fqdn}/api/v1/case/{case_id}", headers=theHive_headers
    )
    if response.status_code != 204:
        LOGGER.error(
            f"Error: Cannot delete '{case_id}' case: {response.status_code}. {response.text}"
        )
...
def thehive_list_case_responders(case_id: str) -> list[dict]:
    ...
    response = requests.get(
        url=f"{theHive_fqdn}/api/connector/cortex/responder/case/{case_id}",
        headers=theHive_headers,
    )
    if response.status_code != 200:
        LOGGER.error(
...
def run_responder():
    ...
    if cortex_responder is None:
        ...
        cortex_responder = cortex_update_responder_conf(
            conf=responder_conf, responder_id=cortex_responder.get("id")
        )
    ...
    responder = None
    for r in responders:
        if r.get("name") == responder_name and r.get("id") == cortex_responder.get(
            "id"
        ):
            responder = r
    ...
    if responder is not None:
        ...
        response = requests.post(
            ...
            headers=theHive_headers,
            json={
                "responderId": responder.get("id"),
                "objectId": case.get("_id"),
                "objectType": case.get("_type"),
            },
        )
        if response.status_code != 200:
            LOGGER.error(
                f"Error: Cannot run '{responder_name}' responder : {response.status_code}. {response.text}"
            )
        LOGGER.info(
            f"Script execution is a success with the following parameters: domain '{domain_name}' and minutes '{minutes}'"
        )
    else:
