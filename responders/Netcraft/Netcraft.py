...
class NetcraftReporter(Responder):
    def __init__(self):
        ...
        self.useUserPass = self.get_param(
            "config.useUserPass",
            None,
            "Takedown Use Username Password authentication is empty",
        )
        ...
        try:
            ...
            if self.observable_type in supported_observables:
                ...
                if response.status_code == 200:
                    self.report(
                        {
                            "message": "Takedown request sent to Netcraft. Message: {}".format(
                                response.text
                            )
                        }
                    )
                    ...
                    self.error(
                        "Failed authentication. Check API-Key. Message: {}".format(
                            response.text
                        )
                    )
                else:
                    self.error(
                        "Failed to submit takedown request. Error code: {}. Error message: {}".format(
