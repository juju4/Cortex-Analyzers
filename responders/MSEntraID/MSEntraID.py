...
class MSEntraID(Responder):
    ...
    def check_user_status(self, user, headers, base_url):
        r = requests.get(
            f"{base_url}users/{user}?$select=accountEnabled", headers=headers
        )
    ...
    def run(self):
        ...
        if self.service == "revokeSignInSessions":
            if self.get_param("data.dataType") == "mail":
                try:
                    self.user = self.get_param(
                        "data.data", None, "No UPN supplied to revoke credentials for"
                    )
                    if not self.user:
                    ...
                    r = requests.post(
                        f"{base_url}users/{self.guid}/revokeSignInSessions",
                        headers=headers,
                    )

                    if r.status_code != 200:
                        self.error(
                            f"Failure to revoke access tokens of user {self.user}: {r.content}"
                        )
                    else:
                ...
                full_report = {
                    "message": f"All refresh tokens and browser session cookies successfully revoked for user {self.user} at {self.time}"
                }
            ...
            if self.get_param("data.dataType") == "mail":
                try:
                    self.user = self.get_param(
                        "data.data", None, "No UPN supplied for password reset"
                    )
                    if not self.user:
                        self.error("No user supplied")
                    self.ensure_guid(headers, base_url)
                    ...
                    r = requests.patch(
                        f"{base_url}users/{self.guid}", headers=headers, json=data
                    )

                    if r.status_code != 204:
                        self.error(
                            f"Failure to reset password for user {self.user}: {r.content}"
                        )

                    self.report(
                        {
                            "message": f"Password reset initiated for user {self.user}, user will be prompted to change it at next sign-in"
                        }
                    )
                except Exception:
                    self.error(traceback.format_exc())
            ...
            try:
                self.user = self.get_param(
                    "data.data", None, "No UPN supplied for password reset with MFA"
                )
                if not self.user:
                    self.error("No user supplied")
                self.ensure_guid(headers, base_url)
                ...
                if r.status_code != 204:
                    self.error(
                        f"Failure to reset password with MFA for user {self.user}: {r.content}"
                    )

                self.report(
                    {
                        "message": f"Password reset initiated for user {self.user}, user will be prompted for MFA and password change at next sign-in"
                    }
                )
            except Exception:
                self.error(traceback.format_exc())
            ...
            if self.get_param("data.dataType") == "mail":
                try:
                    self.user = self.get_param(
                        "data.data", None, "No UPN supplied to enable user"
                    )
                    if not self.user:
                        self.error("No user supplied")
                    self.ensure_guid(headers, base_url)
            ...
            if self.get_param("data.dataType") == "mail":
                try:
                    self.user = self.get_param(
                        "data.data", None, "No UPN supplied to disable user"
                    )
                    if not self.user:
                        self.error("No user supplied")
                    self.ensure_guid(headers, base_url)
                    ...
                    if user_status is False:
                        self.report(
                            {"message": f"User {self.user} is already disabled"}
                        )
                        return
