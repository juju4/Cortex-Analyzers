...
class CloseAlert(Responder):
    def __init__(self):
        ...
        self.zfAction = "request_takedown"

    def operations(self, raw):
        return [
            self.build_operation(
                "AddTagToCase", tag="TheHive:Responders=Zerofox Alert Closed"
            )
        ]
    ...
    def run(self):
        ...
        action_request = "{}/{}/{}/{}/".format(
            self.url, self.zfEntity, self.ZerofoxAlert(tags), self.zfAction
        )
            ...
            if self.ZerofoxAlert(tags):
                try:
                    ...
                    if response.status_code == 200:
                        self.report(
                            {
                                "message": "Alert {} has been closed".format(
                                    self.ZerofoxAlert(tags)
                                )
                            }
                        )
