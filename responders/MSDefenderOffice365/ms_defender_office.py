...
class MsDefenderOffice365Responder(Responder):
    def __init__(self):
        ...
        self.certificate_base64 = self.get_param(
            "config.certificate_base64", None, "Config is missing certificate_base64"
        )
        ...
        self.app_id = self.get_param("config.app_id", None, "Config is missing app_id")
        self.organization = self.get_param(
            "config.organization", None, "Config missing organization"
        )
        self.block_expiration_days = self.get_param("config.block_expiration_days", 0)
    ...
    def run(self):
        ...
        try:
            result = subprocess.run(process_args, capture_output=True)
        except subprocess.TimeoutExpired:
            self.error(
                f"Timeout waiting for {script_name} to complete."
        ...
        try:
            ...
            if match_json_list is not None:
                ...
                self.error("Failed to identify JSON in script output:" + scriptResult)
        except json.JSONDecodeError as e:
            self.error(f"Error decoding JSON: {e}| Input: {extractedJson}")
        ...
        for item in scriptResultDict:
            if item.get("error") is not None:
                ...
                if "Entry not found" in item["error"]:
                    successful_entries.append(f"{item['entry']}: Entry not found.")
                else:
                    error_entries.append(f"{item['entry']}: {item['error']}")
            else:
                success_dict = json.loads(item["result"])
                if self.service == "block":
                    result_msg = f"{item['entry']} Expiration " + str(
                        success_dict["ExpirationDate"]
                    )
                else:
