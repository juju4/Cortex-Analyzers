...
class Block_port(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param(
            "config.Security_rule_for_block_port_external_communication",
            "TheHive Block port for external communication",
        )
    ...
    def run(self):
        ...
        if f"thehive-{port}-{protocol}" not in str(
            fw.find(f"thehive-{port}-{protocol}", panos.objects.ServiceObject)
        ):
            new_port_object = panos.objects.ServiceObject(
                f"thehive-{port}-{protocol}",
                protocol,
                description="TheHive Blocked port",
        ...
        block_list = fw.find(
            "TheHive Block list for external port communication",
        ...
        self.report(
            {
                "message": "Responder successfully added %s into TheHive Block list for external port communication to %s"
                % (port, self.name_security_rule)
            }
        )
