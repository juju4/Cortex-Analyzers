...
class Block_domain(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param(
            "config.Security_rule_for_block_internal_domain",
            "TheHive Block internal Domain",
        )
    ...
    def run(self):
        ...
        if f"thehive-{ioc}" not in str(
            ...
            new_ioc_object = panos.objects.AddressObject(
                f"thehive-{ioc}", ioc, description="TheHive Blocked domain", type="fqdn"
            )
            fw.add(new_ioc_object)
            new_ioc_object.create()
        panos.objects.AddressGroup.refreshall(fw)
        block_list = fw.find(
            "TheHive Block list internal domain", panos.objects.AddressGroup
        )
        if block_list != None:
            ...
            if f"thehive-{ioc}" not in ioc_list:
                ...
                temp1 = panos.objects.AddressGroup(
                    "TheHive Block list internal domain", static_value=ioc_list
                )
                fw.add(temp1)
                temp1.apply()
        elif block_list == None:
            temp1 = panos.objects.AddressGroup(
                "TheHive Block list internal domain", static_value=f"thehive-{ioc}"
            )
            fw.add(temp1)
            temp1.apply()
        ...
        self.report(
            {
                "message": "Responder successfully added %s into TheHive Block list internal domain from %s"
