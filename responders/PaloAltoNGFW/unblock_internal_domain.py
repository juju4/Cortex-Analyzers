...
class Unblock_domain(Responder):
    def __init__(self):
        ...
        self.name_internal_Address_Group_for_domain = self.get_param(
            "config.Address_group_for_unblock_internal_domain",
            "TheHive Block list internal domain",
        )
    ...
    def run(self):
        ...
        block_list = fw.find(
            self.name_internal_Address_Group_for_domain, panos.objects.AddressGroup
        )
        ...
        if f"thehive-{ioc}" in ioc_list:
            ...
            temp1 = panos.objects.AddressGroup(
                self.name_internal_Address_Group_for_domain, static_value=ioc_list
            )
            fw.add(temp1)
            temp1.apply()

        panos.objects.AddressObject.refreshall(fw)
        if f"thehive-{ioc}" in str(
            ...
            try:
                ...
                self.report(
                    {"message": "Responder did not comlite. Warning in AddressObject"}
                )

        self.report(
            {
                "message": "Responder successfully deleted %s from %s"
                % (ioc, self.name_internal_Address_Group_for_domain)
            }
        )
        fw.commit()
