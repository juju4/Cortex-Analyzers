...
class Unblock_domain(Responder):
    def __init__(self):
        ...
        self.name_external_Address_Group_for_domain = self.get_param(
            "config.Address_group_for_unblock_external_domain",
            "TheHive Block list external domain",
        )
    ...
    def run(self):
        ...
        block_list = fw.find(
            self.name_external_Address_Group_for_domain, panos.objects.AddressGroup
        )
        ...
        if f"thehive-{ioc}" in ioc_list:
            ...
            temp1 = panos.objects.AddressGroup(
                self.name_external_Address_Group_for_domain, static_value=ioc_list
            )
            fw.add(temp1)
            temp1.apply()
        ...
        self.report(
            {
                ...
                % (f"thehive-{ioc}", self.name_external_Address_Group_for_domain)
            }
        )
        fw.commit()
