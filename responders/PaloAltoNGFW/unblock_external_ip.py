...
class Unblock_ip(Responder):
    def __init__(self):
        ...
        self.name_external_Address_Group = self.get_param(
            "config.Address_group_for_unblock_external_IP_address",
    ...
    def run(self):
        ...
        block_list = fw.find(
            self.name_external_Address_Group, panos.objects.AddressGroup
        )
        ...
        self.report(
            {
                ...
                % (f"thehive-{ioc}", self.name_external_Address_Group)
            }
        )
        fw.commit()
