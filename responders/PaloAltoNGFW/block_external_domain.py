...
class Block_domain(Responder):
    def __init__(self):
        ...
        self.name_security_rule = self.get_param(
            "config.Security_rule_for_block_external_domain",
            "TheHive Block external Domain",
        )
    ...
    def run(self):
        ...
        block_list = fw.find(
            "TheHive Block list external domain", panos.objects.AddressGroup
        )
        if block_list != None:
            ...
            if f"thehive-{ioc}" not in ioc_list:
                ...
                temp1 = panos.objects.AddressGroup(
                    "TheHive Block list external domain", static_value=ioc_list
                )
                fw.add(temp1)
                temp1.apply()
        elif block_list == None:
            temp1 = panos.objects.AddressGroup(
                "TheHive Block list external domain", static_value=f"thehive-{ioc}"
            )
            fw.add(temp1)
            temp1.apply()
        ...
        self.report(
            {
                "message": "Responder successfully added %s into TheHive Block list external domain from %s"
