...
class Unblock_port(Responder):
    def __init__(self):
        ...
        self.name_external_Service_Group = self.get_param(
            "config.Service_group_for_unblock_external_port",
    ...
    def run(self):
        ...
        block_list = fw.find(
            self.name_external_Service_Group, panos.objects.ServiceGroup
        )
        ...
        self.report(
            {
                ...
                % (f"thehive-{port}-{protocol}", self.name_external_Service_Group)
            }
        )
        fw.commit()
