...
class Unblock_ip(Responder):
    def __init__(self):
        ...
        self.name_internal_Address_Group = self.get_param(
            "config.Address_group_for_unblock_internal_IP_address",
            "TheHive Block list internal IP address",
        )
    ...
    def run(self):
        ...
        block_list = fw.find(
            self.name_internal_Address_Group, panos.objects.AddressGroup
        )
        ...
        if f"thehive-{ioc}" in ioc_list:
            ...
            temp1 = panos.objects.AddressGroup(
                self.name_internal_Address_Group, static_value=ioc_list
            )
            fw.add(temp1)
            temp1.apply()
        ...
        self.report(
            {
                ...
                % (f"thehive-{ioc}", self.name_internal_Address_Group)
            }
        )
        fw.commit()
