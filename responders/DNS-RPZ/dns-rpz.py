...
class DNSRPZ(Responder):
    ...
    def __init__(self):
        ...
        self.tsig_keyval = self.get_param(
            "config.tsig_keyval", None, "Missing TSIG key"
        )
    ...
    def run(self):
        ...
        if self.checkname(domain + "." + self.rpz_zonename) == 0:
            ...
            keyring = dns.tsigkeyring.from_text({self.tsig_keyname: self.tsig_keyval})
            update = dns.update.Update(
                self.rpz_zonename, keyring=keyring, keyalgorithm=self.tsig_hashalg
            )
