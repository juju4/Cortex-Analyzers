...
class MSDefenderEndpoints(Responder):
    def __init__(self):
        ...
        self.msdefenderTenantId = self.get_param(
            "config.tenantId", None, "TenantId missing!"
        )
        self.msdefenderAppId = self.get_param("config.appId", None, "AppId missing!")
        self.msdefenderSecret = self.get_param(
            "config.appSecret", None, "AppSecret missing!"
        )
        self.msdefenderResourceAppIdUri = self.get_param(
            "config.resourceAppIdUri", None, "resourceAppIdUri missing!"
        )
        self.msdefenderOAuthUri = self.get_param(
            "config.oAuthUri", None, "oAuthUri missing!"
        )

        self.observable = self.get_param("data.data", None, "Data is empty")
        self.observableType = self.get_param(
            "data.dataType", None, "Data type is empty"
        )
        self.caseId = self.get_param("data.case.caseId", None, "caseId is missing")
        self.caseTitle = self.get_param(
            "data.case.title", None, "Case title is missing"
        ).encode("utf-8")
    ...
    def run(self):
        ...
        try:
            ...
            self.error(
                {"message": "HTTP ErrorCode {}. Reason: {}".format(e.code, e.reason)}
            )
        ...
        def getMachineId(id):
            ...
            if self.observableType == "ip":
                url = "https://api.securitycenter.windows.com/api/machines/findbyip(ip='{}',timestamp={})".format(
                    id, time
                )
            else:
                url = "https://api.securitycenter.windows.com/api/machines?$filter=computerDnsName+eq+'{}'".format(
                    id
                )
        ...
        def isolateMachine(machineId):
            ...
            """
            url = (
                "https://api.securitycenter.windows.com/api/machines/{}/isolate".format(
                    machineId
                )
            )
        ...
        def runFullVirusScan(machineId):
            ...
            """
            url = "https://api.securitycenter.windows.com/api/machines/{}/runAntiVirusScan".format(
                machineId
            )

            body = {
                "Comment": "Full scan to machine due to TheHive case {}".format(
                    self.caseId
                ),
                "ScanType": "Full",
            }
            ...
            try:
                ...
                if response.status_code == 201:
                    self.report(
                        {
                            "message": "Started full VirusScan on machine: "
                            + self.observable
                        }
                    )
                elif (
                    ...
                    self.report(
                        {
                            "message": "Error full VirusScan on machine: ActiveRequestAlreadyExists"
                        }
                    )
                else:
        ...
        def unisolateMachine(machineId):
            ...
            """
            url = "https://api.securitycenter.windows.com/api/machines/{}/unisolate".format(
                machineId
            )
            body = {
                "Comment": "Unisolate machine due to TheHive case {}".format(
                    self.caseId
                )
            }
        ...
        def restrictAppExecution(machineId):
            ...
            """
            url = "https://api.securitycenter.windows.com/api/machines/{}/restrictCodeExecution".format(
                machineId
            )
            body = {
                "Comment": "Restrict code execution due to TheHive case {}".format(
                    self.caseId
                )
            }
            ...
            try:
                ...
                if response.status_code == 201:
                    self.report(
                        {
                            "message": "Restricted app execution on machine: "
                            + self.observable
                        }
                    )
                elif (
                    ...
                    self.report(
                        {
                            "message": "Error restricting app execution on machine: ActiveRequestAlreadyExists"
                        }
                    )
                else:
        ...
        def unrestrictAppExecution(machineId):
            ...
            """
            url = "https://api.securitycenter.windows.com/api/machines/{}/unrestrictCodeExecution".format(
                machineId
            )
            body = {
                "Comment": '"Remove code execution restriction since machine was cleaned and validated due to TheHive case {}'.format(
                    self.caseId
                )
            }
            ...
            try:
                ...
                if response.status_code == 201:
                    self.report(
                        {
                            "message": "Removed app execution restriction on machine: "
                            + self.observable
                        }
                    )
                elif (
                    ...
                    self.report(
                        {
                            "message": "Error removing app execution restriction on machine: ActiveRequestAlreadyExists"
                        }
                    )
                else:
        ...
        def startAutoInvestigation(machineId):
            ...
            """
            url = "https://api.securitycenter.windows.com/api/machines/{}/startInvestigation".format(
                machineId
            )

            body = {
                "Comment": "Start investigation due to TheHive case {}".format(
                    self.caseId
                )
            }
            ...
            try:
                ...
                if response.status_code == 201:
                    self.report(
                        {
                            "message": "Started Auto Investigation on : "
                            + self.observable
                        }
                    )
                elif (
                    ...
                    self.report(
                        {
                            "message": "Error lauching auto investigation on machine: ActiveRequestAlreadyExists"
                        }
                    )
                else:
        ...
        def pushCustomIocAlert(observable):
            if self.observableType == "ip":
                ...
                self.error(
                    {"message": "Observable type must be ip, url, domain or hash"}
                )
            ...
            try:
                ...
                if response.status_code == 200:
                    self.report(
                        {
                            "message": "Added IOC to Defender with Alert mode: "
                            + self.observable
                        }
                    )
            except requests.exceptions.RequestException as e:
        ...
        def pushCustomIocBlock(observable):
            ...
            try:
                ...
                if response.status_code == 200:
                    self.report(
                        {
                            "message": "Added IOC to Defender with Alert and Block mode: "
                            + self.observable
                        }
                    )
            except requests.exceptions.RequestException as e:
    ...
    def operations(self, raw):
        ...
        if self.service == "isolateMachine":
            ...
            return [
                self.build_operation("AddTagToArtifact", tag="MsDefender:fullVirusScan")
            ]
        elif self.service == "unisolateMachine":
            return [
                self.build_operation("AddTagToArtifact", tag="MsDefender:unIsolated")
            ]
        elif self.service == "restrictAppExecution":
            return [
                self.build_operation(
                    "AddTagToArtifact", tag="MsDefender:restrictedAppExec"
                )
            ]
        elif self.service == "unrestrictAppExecution":
            return [
                self.build_operation(
                    "AddTagToArtifact", tag="MsDefender:unrestrictedAppExec"
                )
            ]
