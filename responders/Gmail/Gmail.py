...
class Gmail(Responder):
    def __init__(self):
        ...
        self.__thehive_url = self.get_param(
            "config.thehive_url", None, "The Hive URL missing"
        )
        self.__thehive_api_key = self.get_param(
            "config.thehive_api_key", None, "The Hive API key missing"
        )
        self.__gmail_domain = self.get_param(
            "config.gmail_domain", None, "The Hive API key missing"
        )
        self.__gmail_service_account = {
            "type": "service_account",
            "project_id": self.get_param(
                "config.gmail_project_id", None, "Project ID missing"
            ),
            "private_key_id": self.get_param(
                "config.gmail_private_key_id", None, "Private Key ID missing"
            ),
            "private_key": self.get_param(
                "config.gmail_private_key", None, "Private Key (PEM format) missing"
            ).replace("\\n", "\n"),
            "client_email": self.get_param(
                "config.gmail_client_email", None, "Client email missing"
            ),
            "client_id": self.get_param(
                "config.gmail_client_id", None, "Client id missing"
            ),
    ...
    def __get_gmail_subjects(self, caseId, query):
        ...
        if response.status_code == 200:
            ...
            self.error(
                "Failed to get valid response for query: {}".format(
    ...
    def hive_auth(self, url, api_key):
        ...
        if not self.hive_check_permissions(user_object):
            self.error(
                "API key of `{}` is missing `read` or `write` role. Users current roles: {}".format(
                    user_object["name"], user_object["roles"]
                )
            )
    ...
    def gmail_impersonate(self, subject):
        ...
        credentials = service_account.Credentials.from_service_account_info(
            info=self.__gmail_service_account, scopes=self.__scopes, subject=subject
        )
    ...
    def trash_message(self, case_id, query):
        ...
        gmail_observables = self.__get_gmail_subjects(
            case_id, And(Eq("dataType", "mail"), EndsWith("data", self.__gmail_domain))
        )
        for observable in gmail_observables:
            ...
            try:
                response = (
                    resource.users()
                    .messages()
                    .list(userId=observable["data"], q=query)
                    .execute()
                )
                for message in response.get("messages", []):
                    resource.users().messages().delete(
                        userId=observable["data"], id=message["id"]
                    ).execute()
        ...
        for observable in gmail_observables:
            ...
            try:
                gmail_filter = (
                    ...
                    .settings()
                    .filters()
                    .create(userId=observable["data"], body=new_filter)
                    .execute()
                )
            except GoogleAuthError as e:
                self.error("Gmail oauth failed: {}".format(e))
    ...
    def unblock_messages(self, case_id):
        ...
        gmail_observables = self.__get_gmail_subjects(
            case_id,
            query=And(
                Eq("dataType", "mail"), And(EndsWith("data", self.__gmail_domain))
            ),
        )
        for observable in gmail_observables:
            ...
            try:
                ...
                resource.users().settings().filters().delete(
                    userId=observable["data"], id=tag.split(":")[-1]
                ).execute()
            except GoogleAuthError as e:
                self.error("Gmail oauth failed: {}".format(e))
            except HttpError as e:
    ...
    def unblockdomain(self, observable, dataType, caseId):
        if dataType != "domain":
            self.error(
                "{} needs data of type 'domain' but {} given".format(
                    self.get_param("config.service"), dataType
                )
            )
    ...
    def unblocksender(self, observable, dataType, caseId):
        if dataType != "mail":
            self.error(
                "{} needs data of type 'mail' but {} given".format(
                    self.get_param("config.service"), dataType
                )
            )
        self.unblock_messages(caseId)
    ...
    def run(self):
        ...
        action = getattr(
            self, self.service, self.__not_found
        )  # call respective func or fail with default
