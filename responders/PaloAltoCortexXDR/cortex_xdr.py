...
class PaloAltoCortexXDRResponder(Responder):
    def __init__(self):
        ...
        self.api_host = self.get_param("config.api_host", None, "Missing API host")
        ...
        self.scan_polling_interval = self.get_param("config.scan_polling_interval", 30)
        ...
        self.service = self.get_param("config.service", None, "Missing config.service")
    ...
    def _check_for_api_errors(
        ...
        if response.status_code != good_status_code:
            ...
            message = error_prefix + "Error {err_code}: {err_msg}".format(**reply)
    ...
    def _make_api_request(self, method: str, error_prefix="", **kwargs):
        ...
        if "reply" not in response_json:
            self.error(
                f"Missing reply in response. Did the API change? {response_json=}"
            )
    ...
    def get_endpoints(
        self, filter_field: Literal["ip_list", "hostname"], data: List[str]
    ) -> GetEndpointResponse:
        ...
        response_json = self._make_api_request(
            ...
            json={
                "request_data": {
                    "filters": [
                        {"field": filter_field, "operator": "in", "value": data}
                    ]
                }
            },
        )
        self.current_endpoints = response_json.get("reply", {}).get("endpoints")
    ...
    def scan_endpoints(self, endpoints: List[Endpoint]):
        ...
        for e in endpoints:
            if e["scan_status"] not in [
                ...
                endpoint_log.append("Found scannable endpoint: " + e["endpoint_id"])
            else:
                unscannable_endpoints.append(e)
                endpoint_log.append(
                    "Found endpoint with scan already in progress: " + e["endpoint_id"]
                )
        ...
        if len(endpoint_ids) == 0:
            return self.report(
                {
                    "success": True,
                    "message": ("No endpoints to scan or scans are already pending."),
    ...
    def get_action_status(self, action_id):
        ...
        response_json = self._make_api_request(
            "post", url=url, json={"request_data": {"group_action_id": action_id}}
        )
    ...
    def poll_action_status(self, action_id, action_type):
        ...
        tries = 0
        while not action_status or (
            tries < max_tries
            and not all(
                [status in terminal_statuses for status in action_status.values()]
            )
        ):
            time.sleep(interval)
    ...
    def run(self):
        ...
        if self.service != "scan" and len(o_data) > 1 and not self.allow_multi_target:
            self.error(
        ...
        if self.service in ["isolate", "unisolate"]:
            if len(endpoints) == 0:
                self.error(
                    "Could not find any endpoints searching"
        ...
        if self.service in ["isolate", "unisolate"] and len(actionable_endpoints) == 0:
            self.report(
                {
                    "success": True,
                    "message": (
                        "No action taken because endpoints are already"
