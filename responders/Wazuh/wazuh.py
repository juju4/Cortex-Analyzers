...
class Wazuh(Responder):
    def __init__(self):
        ...
        self.wazuh_manager = self.get_param(
            "config.wazuh_manager", None, "https://localhost:55000"
        )
        self.wazuh_user = self.get_param("config.wazuh_user", None, "Username missing!")
        self.wazuh_password = self.get_param(
            "config.wazuh_password", None, "Password missing!"
        )
        self.wazuh_agent_id = self.get_param(
            "data.case.customFields.wazuh_agent_id.string", None, "Agent ID Missing!"
        )
        self.wazuh_alert_id = self.get_param(
            "data.case.customFields.wazuh_alert_id.string", None, "Alert ID Missing!"
        )
        self.wazuh_rule_id = self.get_param(
            "data.case.customFields.wazuh_rule_id.string", None, "Rule ID Missing!"
        )
    ...
    def run(self):
        ...
        if self.observable_type == "ip":
            ...
            self.error({"message": "Not a valid IPv4/IPv6 address!"})
        payload = (
            '{"command":"firewall-drop.sh", "arguments": ["-", "'
            + self.observable
            + '", "'
            + self.wazuh_alert_id
            + '", "'
        ...
        r = requests.put(
            self.wazuh_manager + "/active-response/" + self.wazuh_agent_id,
            headers=headers,
            data=payload,
            verify=False,
            auth=auth,
