...
class Minemeld(Responder):
    def __init__(self):
        ...
        self.minemeld_user = self.get_param(
            "config.minemeld_user", None, "Username missing!"
        )
        self.minemeld_password = self.get_param(
            "config.minemeld_password", None, "Password missing!"
        )
        ...
        self.minemeld_confidence = self.get_param(
            "config.minemeld_confidence", None, "Confidence level missing!"
        )
        ...
        self.observable_description = self.get_param(
            "data.message", None, "Message is empty"
        )
    ...
    def run(self):
        ...
        try:
            r = requests.post(
                str(self.minemeld_url)
                + "/config/data/"
                + str(self.minemeld_indicator_list)
                + "_indicators"
                + "/append?h="
                + str(self.minemeld_indicator_list)
                + "&t=localdb",
            ...
            self.report(
                {"message": "Indicator " + self.observable + " submitted to Minemeld."}
            )
        except:
