...
class BackscatterAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.api_key = self.get_param(
            "config.key", None, "No Backscatter.io API key provided."
        )
        self.proxies = {
        ...
        self.service = self.get_param(
            "config.service", None, "Backscatter service is missing"
        )
        ...
        if self.service == "observations":
            ...
            taxonomies = taxonomies + [
                self.build_taxonomy(
                    ...
                    namespace,
                    "Observations",
                    summary.get("observations_count", 0),
                ),
                self.build_taxonomy(
                    level, namespace, "IP Addresses", summary.get("ip_address_count", 0)
                ),
                self.build_taxonomy(
                    level, namespace, "Networks", summary.get("network_count", 0)
                ),
                self.build_taxonomy(
                    level, namespace, "AS", summary.get("autonomous_system_count", 0)
                ),
                self.build_taxonomy(
                    level, namespace, "Ports", summary.get("port_count", 0)
                ),
                self.build_taxonomy(
                    level, namespace, "Protocols", summary.get("protocol_count", 0)
                ),
            ]
        elif self.service == "enrichment":
            ...
            if self.data_type == "ip":
                taxonomies = taxonomies + [
                    self.build_taxonomy(
                        level, namespace, "Network", summary.get("network")
                    ),
                    self.build_taxonomy(
                        level,
                        namespace,
                        "Network Broadcast",
                        summary.get("network_broadcast"),
                    ),
                    self.build_taxonomy(
                        level, namespace, "Network Size", summary.get("network_size")
                    ),
                    self.build_taxonomy(
                        level, namespace, "Country", summary.get("country_name")
                    ),
                    self.build_taxonomy(
                        level, namespace, "AS Number", summary.get("as_num")
                    ),
                    self.build_taxonomy(
                        level, namespace, "AS Name", summary.get("as_name")
                    ),
                ]
            elif self.data_type == "network":
                taxonomies = taxonomies + [
                ...
                taxonomies = taxonomies + [
                    self.build_taxonomy(
                        level, namespace, "Prefix Count", summary.get("prefix_count")
                    ),
                    self.build_taxonomy(
                        level, namespace, "AS Number", summary.get("as_num")
                    ),
                ...
                for result in raw.get("results", list()):
                    ...
                    taxonomies.append(
                        self.build_taxonomy(level, namespace, "Service", display)
                    )
            else:
                pass
        else:
