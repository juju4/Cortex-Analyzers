...
class ProofpointForensicsAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.service = self.get_param(
            "config.service", None, "Proofpoint service is missing"
        )
        self.url = self.get_param(
            "config.url", "https://tap-api-v2.proofpoint.com", None
        )
        self.apikey = self.get_param(
            "config.apikey", None, "Proofpoint apikey is missing"
        )
        self.secret = self.get_param(
            "config.secret", None, "Proofpoint secret is missing"
        )
    ...
    def summary(self, raw):
        ...
        result = {"service": self.service, "dataType": self.data_type}
        if "reports" in raw:
