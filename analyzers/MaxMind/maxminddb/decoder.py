...
class Decoder(object):  # pylint: disable=too-few-public-methods
    ...
    def _decode_pointer(self, size, offset):
        ...
        packed = (
            pointer_bytes
            if pointer_size == 4
            else struct.pack(b"!c", byte_from_int(size & 0x7)) + pointer_bytes
        )
        unpacked = int_from_bytes(packed)
        pointer = (
            unpacked + self._pointer_base + self._pointer_value_offset[pointer_size]
        )
    ...
    def _size_from_ctrl_byte(self, ctrl_byte, offset, type_num):
        ...
        if size == 29:
            ...
            size = struct.unpack(b"!I", size_bytes.rjust(4, b"\x00"))[0] + 65821
