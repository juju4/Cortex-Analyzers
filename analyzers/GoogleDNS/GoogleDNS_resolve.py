...
class GoogleDNS_resolve(Analyzer):
        ...
        try:
            ...
            if data["Status"] == 0:  # DNS response code
                    ...
                    for records in data["Answer"]:  # for each records found by Google
                        try:
                            records["type"] = RECORDS[
                                records["type"]
                            ]  # replace IANA code by record name
                        except KeyError:
                            data["Error"] = "Invalid IANA code : {0}".format(
                                int(records["type"])
                            )  # Maybe using a special code
                else:
                    data["Answer"] = []
                ...
                try:
                    ...
                    data["Error"] = "Error for {0} : {1}".format(
                        data["Question"][0]["name"], CODE[int(data["Status"])]
                    )
                except KeyError:
            ...
            self.answer = data
            self.answer["Question"][0]["type"] = RECORDS[
                data["Question"][0]["type"]
            ]  # eplace IANA code by record name
            self.answer["Status"] = CODE[
                int(data["Status"])
            ]  # replace DNS response code by name
    ...
    def run(self):
        ...
        target = (
            ".".join(target.split(".")[::-1]) + ".in-addr.arpa"
    ...
    def summary(self, raw):
        count = self.build_taxonomy(
            "info", "GoogleDNS", "RecordsCount", len(self.answer["Answer"])
        )
