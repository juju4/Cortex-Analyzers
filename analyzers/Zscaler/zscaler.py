...
class ZscalerAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.api_key = self.get_param(
            "config.api_key", None, "Zscaler API key is missing"
        )
        self.base_uri = self.get_param(
            "config.base_uri", None, "Zscaler base URI is missing"
        )
        self.username = self.get_param(
            "config.username", None, "Zscaler username is missing"
        )
        self.password = self.get_param(
            "config.password", None, "Zscaler password is missing"
        )
        self.headers = {
    ...
    def summary(self, raw):
        ...
        if "urlClassifications" in raw:
            ...
            if url_classification:
                if set(url_classification).intersection(mal_category):
                    value = ", ".join(
                        set(url_classification).intersection(mal_category)
                    )
                    level = "malicious"
                elif set(url_classification).intersection(sus_category):
                    value = ", ".join(
                        set(url_classification).intersection(sus_category)
                    )
                    level = "suspicious"
                else:
            ...
            if url_sec_classification:
                if set(url_sec_classification).intersection(mal_category):
                    value = ", ".join(
                        set(url_sec_classification).intersection(mal_category)
                    )
                    level = "malicious"
        ...
        taxonomies.append(
            self.build_taxonomy(level, "Zscaler", "Classification", value)
        )
    ...
    def run(self):
        ...
        if (
            ...
            if self.data_type == "url":
                ...
                data = (
                    url_data.netloc + url_data.path + url_data.params + url_data.query
                )
            ...
            r = session.post(
                self.base_uri + "/api/v1/authenticatedSession",
                headers=self.headers,
                json=payload,
            )
            s = session.post(
                self.base_uri + "/api/v1/urlLookup",
                headers=self.headers,
                json=url_to_query,
            )
            s.close()
