...
class IBMXForceAnalyzer(Analyzer):
    ...
    def cleanup(self, ip_data={}, malware_data={}, dns_data={}, whois_data={}):
        ...
        if self.data_type == "ip":
            ...
            score = (
                "%d [%d category(s)]" % (score_value, len(cats))
                if len(cats) > 0
                else score_value
            )
            families = []
            ...
            families = (
                malware_data.get("malware")
                .get("origins")
                .get("external")
                .get("family", [])
            )
            families = (
                families if families is not None and isinstance(families, list) else []
            )
            detectionCoverage = (
                malware_data.get("malware")
                .get("origins")
                .get("external")
                .get("detectionCoverage")
            )
            score = (
                "%s [%d family(s)]" % (score_value, len(families))
                if len(families) > 0
                else score_value
            )
            cats = []
        ...
        for hist in ip_data.get("history", []):
            ...
            tmp["cats"] = [
                {"name": x, "score": y} for (x, y) in hist.get("cats", {}).items()
            ]
        ...
        if self.data_type != "hash":
            ...
            response["emails_info"] = [
                (x["domain"], x["ip"], x["uri"], self.parse_data(x["lastseen"]))
                for x in emails
            ]
            response["subjects_info"] = [
                (x["subject"], x["count"], x["ips"], self.parse_data(x["lastseen"]))
                for x in subjects
            ]
        ...
        if "RDNS" in dns_data.keys():
            response["dns"].append(("", "", ",".join([x for x in dns_data["RDNS"]])))
    ...
    def ip_query(self, data):
        ...
        try:
            ...
            _query_ip = _session.get(
                "%s/ipr/%s" % (self.url, data), proxies=self.proxies, verify=self.verify
            )
            ...
            _query_info = _session.get(
                "%s/resolve/%s" % (self.url, data),
                proxies=self.proxies,
                verify=self.verify,
            )
            _query_whois = _session.get(
                "%s/whois/%s" % (self.url, data),
                proxies=self.proxies,
                verify=self.verify,
            )
            ...
            malware_data = (
                _query_malware.json() if _query_malware.status_code == 200 else {}
            )
    ...
    def domain_query(self, data):
        ...
        try:
            ...
            _query_url = _session.get(
                "%s/url/%s" % (self.url, data), proxies=self.proxies, verify=self.verify
            )
            _query_malware = _session.get(
    ...
    def malware_query(self, data):
        ...
        try:
            ...
            if _query_malware.status_code == 200:
                ...
                self.report({"resource": data, "errorMessage": "No such sample found"})
    ...
    def summary(self, raw):
        ...
        taxonomies.append(
            self.build_taxonomy(level, namespace, predicate, "{}".format(score))
        )
