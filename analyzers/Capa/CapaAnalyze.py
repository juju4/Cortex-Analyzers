...
class CapaAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.capa_path = self.get_param(
            "config.capa_path", "/Cortex-Analyzers/analyzers/Capa/capa"
        )
    ...
    def run(self):
        if not os.path.isfile(self.capa_path) or not os.access(self.capa_path, os.X_OK):
            self.error(
                f"capa binary not found or not executable at path: {self.capa_path}"
            )
            return
        ...
        try:
            result = subprocess.run(
                [self.capa_path, "-j", self.filepath], capture_output=True, text=True
            )
            if result.returncode != 0:
                self.error(
                    f"capa execution failed with return code {result.returncode}: {result.stderr}"
                )
                return
