...
class NERDAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.base_url = self.get_param(
            "config.url", None, "Config error: Missing URL of NERD."
        )
        if not self.base_url.endswith("/"):
            self.base_url += "/"
        self.api_key = self.get_param(
            "config.key", None, "Config error: Missing API key for NERD."
        )
    ...
    def summary(self, raw):
        ...
        if "message" in raw:
            ...
            rep = round(raw["rep"], 3)
            rep_level = (
                "safe" if rep < 0.02 else ("suspicious" if rep <= 0.5 else "malicious")
            )
            ...
            if raw["blacklists"]:
                taxonomies.append(
                    self.build_taxonomy(
                        "malicious", "NERD", "Blacklists", len(raw["blacklists"])
                    )
                )
    ...
    def run(self):
        ...
        try:
            ...
            self.error(
                "Unexpected or invalid response received from server (can't parse as JSON). A possible reason can be wrong URL."
            )
            return
        ...
        if resp.status_code == 404 and data.get("error") == "IP address not found":
            ...
            self.report(
                {
                    "rep": 0.0,
                    "message": "{} not found in NERD, i.e. there are no recent reports of malicious activity.".format(
                        ip
                    ),
                    "nerd_url": "{}ip/{}".format(
                        self.base_url, ip
                    ),  # Link to IP's page at NERD web
                }
            )
            return
            ...
            try:
                ...
                translated_tags = []
                tag_ids = [
                    t["n"] for t in data["tags"] if t.get("c", 1.0) >= 0.5
                ]  # List of tags with confidence >= 50%
                for tag in tag_ids:
                    try:
                ...
                self.report(
                    {
                        "rep": data[
                            "rep"
                        ],  # reputation score (number between 0.0 to 1.0)
                        "hostname": data["hostname"],  # result of DNS PTR qeury
                        "asn": data[
                            "asn"
                        ],  # list of ASNs announcing the IP (usually just one)
                        "country": data["geo"].get(
                            "ctry", ""
                        ),  # Geolocation - two-letter country code
                        "blacklists": data[
                            "bl"
                        ],  # List of blacklists the IP is listed on
                ...
                self.error(
                    "Invalid response received from server, missing field: {}".format(e)
                )
        else:
            # Unexpected status code, there should be an 'error' field with error message
            self.error(
                "Error: {} {}".format(
                    resp.status_code, data.get("error", "(no error message)")
                )
            )
            return
