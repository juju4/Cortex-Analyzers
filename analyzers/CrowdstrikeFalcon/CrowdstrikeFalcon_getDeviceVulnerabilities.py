...
class CrowdstrikeFalcon_GetDeviceVulnerabilities(Analyzer):
    ...
    def run(self):
        ...
        if self.data_type == "hostname":
            try:
                ...
                if status_code != 200:
                    self.error(
                        f"No devices found with hostname: {hostname} -- {status_code}"
                    )
                ...
                spotlight = SpotlightVulnerabilities(
                    auth_object=auth, ext_headers=extra_headers
                )
                host_vulns = spotlight.query_vulnerabilities_combined(
            ...
            if len(parts) == 3:
                ...
                if main_key in d and sub_key in d[main_key]:
                    ...
                    for entity in d[main_key][sub_key]:
                        filtered[main_key][sub_key].append(
                            {sub_sub_key: entity[sub_sub_key]}
                        )
            elif len(parts) == 2:
    ...
    def summary(self, raw):
        ...
        # Build summary
        taxonomies.append(
            self.build_taxonomy(level, namespace, predicate, str(count_vulns))
        )
