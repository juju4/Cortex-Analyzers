        ...
        if self.data_type == "hostname":
            try:
                ...
                response = alerts.query_alerts(
                    filter=f"device.hostname:'{hostname}'+product:['epp']+(created_timestamp:>='now-{self.days_before}d'+created_timestamp:<'now')"
                )
                if 200 <= response["status_code"] < 300:
                    alert_ids = response["body"]["resources"]
                else:
                    return self.error(
                        f"Error on getting device alert IDs : {response['body']['errors']}"
                    )
                ...
                if alert_ids:
                    ...
                    if 200 <= alerts_info_response["status_code"] < 300:
                        ...
                        for alert in alerts_info:
                            filtered_alert = {
                                key: alert[key] if key in alert else None
                                for key in self.alert_fields
                            }
                            filtered_alert_list.append(filtered_alert)
                    else:
                        return self.error(
                            f"Something went wrong when getting device alerts {alerts_info_response['body']['errors']}"
                        )
