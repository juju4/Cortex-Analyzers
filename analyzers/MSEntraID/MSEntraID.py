...
class MSEntraID(Analyzer):
    def __init__(self):
        ...
        self.client_id = self.get_param(
            "config.client_id",
            None,
            "Microsoft Entra ID Application ID/Client ID Missing",
        )
        self.client_secret = self.get_param(
            "config.client_secret",
            None,
            "Microsoft Entra ID Registered Application Client Secret Missing",
        )
        self.tenant_id = self.get_param(
            "config.tenant_id", None, "Microsoft Entra ID Tenant ID Mising"
        )
    ...
    def handle_get_signins(self, headers, base_url):
        ...
        try:
            ...
            if r.status_code != 200:
                self.error(
                    f"Failure to pull sign-ins for user {self.user}: {r.content}"
                )

            signins_data = r.json().get("value", [])

            new_json = {"filterParameters": None, "signIns": []}
            ...
            for signin in signins_data:
                ...
                if status_info.get("errorCode") == 0:
                    ...
                    basic_details["result"] = (
                        f"Failure: {failure_reason}" if failure_reason else "Failure"
                    )
                ...
                if success:
                    ...
                    actual_country = (
                        location_details.get("countryOrRegion", "").strip().lower()
                    )
                    ...
                    if (
                        expected_country
                        and actual_country
                        and actual_country != expected_country
                    ):
                        ex_country += 1
    ...
    def handle_get_userinfo(self, headers, base_url):
        ...
        try:
            ...
            params = {"$select": ",".join(self.params_list)}
            ...
            user_details = {
                ...
                "onPremisesLastSyncDateTime": user_data.get(
                    "onPremisesLastSyncDateTime", "N/A"
                ),
                "onPremisesSecurityIdentifier": user_data.get(
                    "onPremisesSecurityIdentifier", "N/A"
                ),
                ...
                "lastSignInDateTime": user_data.get("signInActivity", {}).get(
            ...
            if manager_resp.status_code == 200:
                ...
                if not manager_data.get("error"):
                    user_details["manager"] = {
                        ...
                        "userPrincipalName": manager_data.get(
            ...
            if mfa_r.status_code == 200:
                ...
                for method in mfa_data:
                    ...
                    if "phoneauthenticationmethod" in method_odata_type:
                        ...
                        parsed_method["smsSignInState"] = method.get(
                            "smsSignInState", "N/A"
                        )
                        ...
                        parsed_method["phoneAppVersion"] = method.get(
                            "phoneAppVersion", "N/A"
                        )
                        parsed_method["isDefault"] = method.get("isDefault", "N/A")
                        parsed_method["isRegisteredForPasswordless"] = method.get(
                            "isRegisteredForPasswordless", "N/A"
                        )
                        ...
                        parsed_method["attestationCertificates"] = method.get(
                            "attestationCertificates", []
                        )
                        parsed_method["attestationLevel"] = method.get(
                            "attestationLevel", "N/A"
                        )
                        parsed_method["createdDateTime"] = method.get(
                            "createdDateTime", "N/A"
                        )
                        parsed_method["isSelfServiceRegistration"] = method.get(
                            "isSelfServiceRegistration", "N/A"
                        )
                        parsed_method["isSystemProtected"] = method.get(
                            "isSystemProtected", "N/A"
                        )
                        ...
                        parsed_method["creationDateTime"] = method.get(
                        ...
                        parsed_method["emailAddress"] = method.get(
                            "emailAddress", "N/A"
                        )
                        ...
                        parsed_method["startDateTime"] = method.get(
                            "startDateTime", "N/A"
                        )
                        parsed_method["createdDateTime"] = method.get(
                            "createdDateTime", "N/A"
                        )
                        parsed_method["lifetimeInMinutes"] = method.get(
                            "lifetimeInMinutes", "N/A"
                        )
                        parsed_method["isUsable"] = method.get("isUsable", "N/A")
                        parsed_method["isUsableOnce"] = method.get(
                            "isUsableOnce", "N/A"
                        )
                        ...
                        parsed_method["certificateUserIds"] = method.get(
                            "certificateUserIds", []
                        )
                        parsed_method["createdDateTime"] = method.get(
                            "createdDateTime", "N/A"
    ...
    def handle_get_directoryAuditLogs(self, headers, base_url):
        ...
        try:
            ...
            filter_time = (
                datetime.utcnow() - timedelta(days=self.time_range)
    ...
    def handle_get_devices(self, headers, base_url):
        ...
        try:
            ...
            if self.data_type == "mail":
                ...
                filter_q = (
                    f"(userPrincipalName eq '{safe_upn}' or userId eq '{self.guid}')"
                )
            else:  # hostname
    ...
    def summary(self, raw):
        ...
        if self.service == "getSignIns":
            if len(raw.get("signIns", [])) == 0:
                taxonomies.append(
                    self.build_taxonomy("info", "MSEntraIDSignins", "SignIns", "None")
                )
            else:
                taxonomies.append(
                    self.build_taxonomy(
                        "safe", "MSEntraIDSignins", "Count", len(raw["signIns"])
                    )
                )
            ...
            if stats.get("riskySignIns", 0) != 0:
                taxonomies.append(
                    self.build_taxonomy(
                        "suspicious", "MSEntraIDSignins", "Risky", stats["riskySignIns"]
                    )
                )
            ...
            if raw.get("userPrincipalName"):
                taxonomies.append(
                    self.build_taxonomy(
                        "info", "MSEntraIDUserInfo", "UPN", raw["userPrincipalName"]
                    )
                )
        elif self.service == "getDirectoryAuditLogs":
            ...
            taxonomies.append(
                self.build_taxonomy("info", "MSEntraIDAuditLogs", "count", count)
            )
        elif self.service == "getManagedDevicesInfo":
            # Get the count of devices returned.
            count = len(raw.get("devices", []))
            taxonomies.append(
                self.build_taxonomy("info", "MSEntraIDManagedDevices", "count", count)
            )
