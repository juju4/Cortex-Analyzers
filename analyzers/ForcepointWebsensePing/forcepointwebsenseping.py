...
def summary(self, raw):
    ...
    if raw.get("Categories", None):
        ...
        if value in self.get_param("config.malicious_categories", []):
            ...
            level = "info"
        taxonomies.append(
            self.build_taxonomy(level, "Forcepoint", "WebsensePing", value)
        )
    ...
    def run(self):
        ...
        if self.data_type in ("url", "ip", "domain", "fqdn"):
            ...
            command = "export LD_LIBRARY_PATH={} && {}/WebsensePing -m 25 -url {} -s {} -t {}".format(
                self.path, self.path, data, self.hostname, self.timeout
            )
            ...
            report = {
                k.strip(): v.strip()
                for k, v in [x.split("=") for x in output if x.find("=") != -1]
            }
            self.report(report)
        else:
