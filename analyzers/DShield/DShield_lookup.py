...
class DShieldAnalyzer(Analyzer):
    ...
    def artifacts(self, raw):
        artifacts = []
        if "as" in raw:
            artifacts.append(self.build_artifact("autonomous-system", str(raw["as"])))
        if "asabusecontact" in raw:
            artifacts.append(self.build_artifact("mail", str(raw["asabusecontact"])))
        return artifacts
    ...
    def summary(self, raw):
        ...
        value = "{} count(s) / {} attack(s) / {} threatfeed(s)".format(
            raw["count"], raw["attacks"], raw["threatfeedscount"]
        )
    ...
    def run(self):
        if self.data_type == "ip":
            ...
            if self.data_type in r.keys():
                ...
                results["count"] = (
                    info["count"] if isinstance(info["count"], int) else 0
                )
                results["attacks"] = (
                    info["attacks"] if isinstance(info["attacks"], int) else 0
                )
                results["lastseen"] = (
                    info["maxdate"] if isinstance(info["maxdate"], str) else "None"
                )
                results["firstseen"] = (
                    info["mindate"] if isinstance(info["mindate"], str) else "None"
                )
                results["updated"] = (
                    info["updated"] if isinstance(info["updated"], str) else "None"
                )
                results["comment"] = (
                    info["comment"] if isinstance(info["comment"], str) else "None"
                )
                ...
                if "threatfeeds" not in info:
                    ...
                    results["threatfeedscount"] = len(
                        json.loads(json.dumps(info["threatfeeds"]))
                    )
                ...
                if results["attacks"] > 0:
                    results["maxrisk"] = round(
                        min(math.log10(results["attacks"]) * 2, maxrisk)
                    )

                # We add the number of threat feeds to the maxrisk to increase the detection rate
                results["reputation"] = self.get_reputation(
                    int(results["maxrisk"]) + results["threatfeedscount"]
                )
                self.report(results)
            else:
