...
class NessusAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.url = self.get_param("config.url", None, "Missing Nessus scanner URL")
        ...
        self.ca_bundle = self.get_param("config.ca_bundle")
        self.allowed_networks = self.get_param("config.allowed_networks")
        ...
        if self.allowed_networks is not None:
            if self.data_type == "fqdn":
                ...
                try:
                    ...
                    self.error("{}".format(e))
            if not any(
                address in IPNetwork(network) for network in self.allowed_networks
            ):
                self.error("Invalid target: not in any allowed network")

        scanner_args = {"url": self.url, "login": self.login, "password": self.password}
    ...
    def _run_scan(self, scanner):
        ...
        while running:
            ...
            for scan in scanner.res["scans"]:
                if scan["uuid"] == scan_uuid and (
                    scan["status"] == "running" or scan["status"] == "pending"
                ):
                    time.sleep(2)
                    counter += 2
    ...
    def _delete_scan(self, scanner):
        scanner.action("scans/" + str(scanner.scan_id), method="DELETE")
