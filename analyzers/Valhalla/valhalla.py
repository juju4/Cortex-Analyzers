...
class ValhallaAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.valhalla_key = self.get_param(
            "config.key", None, "Missing Valhalla API key"
        )
    ...
    def check_response(self, response):
        ...
        if status == "error":
            self.error(
                "Query failed: %s Message: %s"
                % (str(status), response.get("message", "not set"))
            )
        results = response
        return results
    ...
    def run(self):
        if self.data_type == "hash":
            ...
            if len(data) == 64:
                ...
                self.report(
                    {"status": "error", "message": "hash is not SHA256", "results": []}
                )
        else:
