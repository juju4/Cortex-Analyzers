...
class IlluminateAnalyzer(Analyzer):
    def __init__(self):
        ...
        self._property = self.get_param(
            "config.property",
            None,
            "RiskIQ Illuminate Analyzer object property is missing",
        )
        self._username = self.get_param(
            "config.username", None, "RiskIQ Illuminate username is missing"
        )
        self._api_key = self.get_param(
            "config.api_key", None, "RiskIQ Illuminate api_key is missing"
        )
        self._days_back = self.get_param(
            "config.days_back", None, "RiskIQ Illuminate days_back is missing"
        )
    ...
    def run(self):
        ...
        try:
            ...
            if e.status_code == 404:
                ...
                self.error(
                    'API error while getting property "{0}": {1}'.format(
                        self._property, e
                    )
                )
        except riqanalyzer.AnalyzerError as e:
            self.error(
                'Analyzer error while getting property "{0}": {1}'.format(
                    self._property, e
                )
            )
        try:
            ...
            self.error(
                'Cannot transform property "{0}" to dictionary: {1}'.format(
                    self._property, e
                )
            )
