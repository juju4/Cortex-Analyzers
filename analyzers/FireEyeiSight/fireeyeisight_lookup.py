...
class FireEyeiSightAnalyzer(Analyzer):
    ...
    def cleanup(self, data_info=[]):
        ...
        for report in data_info:
            identifier = (
                report.get("fileIdentifier", None)
                if self.data_type == "hash"
                else report.get("networkIdentifier", None)
            )
            ...
            for field in json_fields:
                ...
                if field == "observationTime":
                    field_value = datetime.fromtimestamp(field_value).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                if (
                    field_value
                    and field in json_unique_fields
                    and field_value not in response[field]
                ):
        ...
        for identifier in response["identifier"]:
            if identifier in ["Attacker", "Compromised"]:
                ...
                identifier in ["Victim", "Related"] and response["level"] != "malicious"
            ):
                response["level"] = "safe"

        for i in range(len(data_info)):
            response["report_data"].append(
                {
    ...
    def query(self, data):
        ...
        try:
            ...
            report_ids = (
                [x.get("reportId", None) for x in r["message"]] if r != -1 else []
            )
            data_info = []
            for report in report_ids:
                r = self.request_handler.exec_query(
                    "/pivot/report/%s/indicator" % report
                )
                if self.data_type == "domain":
                    tmp_info = (
                        [
                            x
                            for x in r["message"].get("publishedIndicators", [])
                            if x.get("domain", None) == data
                        ]
                        if r != -1
                        else []
                    )
                    ...
                    tmp_info = (
                        [
                            x
                            for x in r["message"].get("publishedIndicators", [])
                            if x.get("md5", None) == data
                            or x.get("sha1", None) == data
                            or x.get("sha256", None) == data
                        ]
                        if r != -1
                        else []
                    )
