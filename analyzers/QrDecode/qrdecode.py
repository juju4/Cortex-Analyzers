...
class QrDecode(Analyzer):
    def __init__(self):
        ...
        # Errors Dictionary
        self.errors = (
            "Unable to read QR code / No QR code detected",
    ...
    def summary(self, raw):
        ...
        if "stats" in raw:
            ...
            if "total_qr_codes" in stats:
                ...
                taxonomies.append(
                    self.build_taxonomy(
                        level, namespace, predicate_totalqr, total_qr_codes
                    )
                )
    ...
    def artifacts(self, raw):
        ...
        if raw["results_list"]:
            for result in raw["results_list"]:
                ...
                if (
                    "results" in result
                    and "data_type" in result["results"]
                    and "data" in result["results"]
                ):
                    ...
                    if data_type == "btc_address":
                        match = re.match(
                            r"bitcoin:((bc1[0-9a-zA-Z]{25,39})|([13][a-km-zA-HJ-NP-Z1-9]{25,34}))",
                            data,
                        )
                        if match:
                            data = match.group(1)
                    ...
                    artifacts.append(
                        self.build_artifact(data_type, data, tags=tags, message=message)
                    )
        return artifacts
    ...
    def operations(self, raw):
        ...
        if raw["results_list"]:
            ...
            operations.append(
                self.build_operation("AddTagToArtifact", tag="Data_Type:" + types[:-1])
            )
    ...
    def qr_decode(self, file):
        ...
        data_categories = (
            "Web_and_Network_Data",
            "Communication_Data",
            "Financial_and_Transactional_Data",
        )
