...
class WildFireAnalyzer(Analyzer):
    @staticmethod
    def get_file_hash(f: BinaryIO, blocksize: int = 8192, algorithm=hashlib.sha256):
        file_hash = algorithm()
    ...
    def _check_for_api_errors(self, response: requests.Response, error_prefix=""):
        ...
        if (
            response.headers["content-type"].lower() == "text/xml"
    ...
    def _listify_path(self, pointer: dict, path_keys: list[str]) -> None:
        ...
        if len(path_keys) == 1:
            ...
            elif current_key in pointer and isinstance(pointer[current_key], dict):
    ...
    def get_verdict(self, data_type: str, data: str) -> Tuple[str, int]:
        ...
        if data_type in ["hash", "file"]:
            ...
            self.error(f"Can't get verdict for unknown data type '{data_type}'")
        try:
        ...
        verdict_int = int(
            xmltodict.parse(response.text)["wildfire"]["get-verdict-info"]["verdict"]
        )
        verdict_str = WildFireAnalyzer._verdicts[verdict_int]
        return verdict_str, verdict_int

    def get_report(self, data_type: str, report_key: str, report_format="xml") -> dict:
        ...
        if data_type == "hash" or data_type == "file":
            request_data = dict(
                apikey=self.api_key, hash=report_key, format=report_format
            )
        else:
        ...
        try:
            ...
            self._check_for_api_errors(
                response, f"While getting report for {data_type=} with {report_key=}:"
            )
        except requests.RequestException as e:
    ...
    def submit_file(self, file_path: str, filename="sample") -> dict:
        ...
        if "wildfire" in response_dict:
            ...
            self.error("Missing 'wildfire' key in response data: " + str(response_dict))
