...
class InoitsuAnalyzer(Analyzer):
    ...
    def inoitsu_check(self, email):
        ...
        if "BREACH DETECTED!" in response:
            ...
            Breached_data_finder = re.search(
                "Breached Personal Data(.*)Critical Identity Alerts", text
            )
            Breached_data = Breached_data_finder.group(1)[1:]
            Critical_data_finder = re.search(
                "Critical Identity Alerts(.*)Total Breaches", text
            )
            Critical_data = Critical_data_finder.group(1)[1:]
            Total_breaches_finder = re.search(
                "Total Breaches(.*)Most Recent Breach", text
            )
            Total_breaches = Total_breaches_finder.group(1)[1:]
            Most_recent_breach_finder = re.search(
                "Most Recent Breach(.*)Relative Exposure Rating", text
            )
            Most_recent_breach = Most_recent_breach_finder.group(1)[2:]
            Exposure_rating_finder = re.search(
                "Relative Exposure Rating(.*)breach data from", text
            )
