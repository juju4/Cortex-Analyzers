...
class EmergingThreatsAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.apikey = self.get_param(
            "config.key", None, "EmergingThreats apikey is missing"
        )
    ...
    def summary(self, raw):
        ...
        if self.data_type in ["domain", "ip"] and raw["reputation"] not in [
            "-",
            "Error",
        ]:
            for x in raw["reputation"]:
                ...
                taxonomies.append(
                    self.build_taxonomy(
                        level, namespace, "%s-info" % self.data_type, value
                    )
                )
        elif self.data_type == "hash" and raw["events"] not in ["-", "Error"]:
            value = str(len(raw["events"])) + " signatures"
            taxonomies.append(
                self.build_taxonomy("malicious", namespace, "malware-info", value)
            )
