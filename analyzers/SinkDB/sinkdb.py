...
class SinkDBAnalyzer(Analyzer):
    def __init__(self):
        ...
        if self.data_type not in ["ip", "domain", "fqdn", "mail"]:
            self.error(
                "SinkDB Analyzer only usable with the ip, domain, and mail data types."
            )

        self.apikey = self.get_param(
            "config.key", None, "HTTPS API Key needed for querying SinkDB."
        )
    ...
    def query_db(self, indicator):
        if self.data_type == "ip":
            return self.parse_entries(
                json.loads(
                    self.do_post(
                        "api_key={}&ipv4={}".format(self.apikey, self.data)
                    ).text
                )
            )
            ...
            return self.parse_entries(
                json.loads(
                    self.do_post(
                        "api_key={}&domain={}".format(self.apikey, self.data)
                    ).text
                )
            )
            ...
            return self.parse_entries(
                json.loads(
                    self.do_post(
                        "api_key={}&email={}".format(self.apikey, self.data)
                    ).text
                )
            )
    ...
    def parse_entries(self, entries):
        ...
        if entries["query_status"] == "ok":
            ...
            self.error(
                "SinkDB did not recognize the IP as valid. Here is the full response:\n{}".format(
                    json.dumps(entries)
                )
            )
        else:
            self.error(
                "There was an unknown error communicating with the SinkDB API. Here is the full response:\n{}".format(
                    json.dumps(entries)
                )
            )
    ...
    def do_post(self, data):
        return requests.post(
            "https://sinkdb-api.abuse.ch/api/v1/",
    ...
    def run(self):
        try:
            ...
            self.error(
                "Error when attempting to retrieve data:\n{}".format(
    ...
    def summary(self, raw):
        ...
        for k, v in raw.iteritems():
            if v:
                taxonomies.append(
                    self.build_taxonomy("suspicious", "SinkDB", k, "True")
                )
            else:
