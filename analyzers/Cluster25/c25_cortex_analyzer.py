...
class Cluster25Client:
    ...
    def _get_cluster25_token(self) -> List[dict]:
    ...
    def investigate(self, indicator) -> dict:
        ...
        r = requests.get(
            url=f"{self.base_url}/investigate", params=params, headers=self.headers
        )
        if r.status_code != 200:
            return {
                "error": f"Unable to retrieve investigate result for indicator '{indicator.get('value')}' "
...
class C25CortexAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.c25_api_key = self.get_param(
            "config.client_key", None, "Missing Cluster25 api key"
        )
        self.c25_client_id = self.get_param(
            "config.client_id", None, "Missing Cluster25 client id"
        )
        self.c25_base_url = self.get_param(
            "config.base_url", None, "Missing Cluster25 base url"
        )
        self.c25_api_client = Cluster25Client(
            self.c25_client_id, self.c25_api_key, self.c25_base_url
        )

    def investigate(self, indicator: str) -> dict:
        return self.c25_api_client.investigate({"value": indicator})

    def summary(self, indicator_data: dict) -> dict:
        taxonomies = []
        namespace = "C25"
        level = "info"
        if indicator_data.get("indicator"):
            taxonomies.append(
                self.build_taxonomy(
                    level, namespace, "Indicator", indicator_data.get("indicator")
                )
            )
        ...
        if indicator_data.get("score"):
            ...
            taxonomies.append(
                self.build_taxonomy(
                    level, namespace, "Score", indicator_data.get("score")
                )
            )
        if len(taxonomies) == 0:
            taxonomies.append(
                self.build_taxonomy(level, namespace, "Threat", "Not found")
            )
