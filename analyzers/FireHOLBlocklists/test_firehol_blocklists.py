...
class TestFireholBlocklistsValidData(unittest.TestCase):
    ...
    def test_path(self):
        self.assertEqual(
            self.analyzer.path, os.path.join(__abspath__, "test_data"), "Wrong path."
        )
    ...
    def test_results(self):
        ...
        self.assertEqual(
            results.get("full").get("count"), 2, "Number of hits are wrong."
        )
        for hit in results.get("full").get("hits"):
            self.assertTrue(
                hit.get("list") == "ips" or hit.get("list") == "net",
                "Expected lists are wrong.",
            )
