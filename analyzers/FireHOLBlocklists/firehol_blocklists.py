...
class FireholBlocklistsAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.path = self.get_param(
            "config.blocklistpath", None, "No path to blocklists provided."
        )
                    ...
                    if l[0] == "#":
                        ...
                        if "# Source File Date: " in l:
                            ...
                            if (self.now - date).days > self.ignoreolderthandays:
                                break
                        description[ipsetname] += re.sub(
                            r"^\[.*\] \(.*\) [a-zA-Z0-9.\- ]*$", "", l.lstrip("# ")
                        ).replace("\n\n", "\n")
                    else:
                        if ip in l:
                            ...
                            hits.append(
                                {
                                    "list": ipsetname,
                                    "description": description.get(ipsetname),
                        ...
                        try:
                            if ipaddress.ip_address(ip) in ipaddress.ip_network(
                                ...
                                hits.append(
                                    {
                                        "list": netsetname,
                                        "description": description.get(netsetname),
                            ...
                            self.error(
                                "ValueError occurred. Used values: ipnetwork {}, ip to check {}, file {}."
