...
class ElasticsearchAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.endpoints = self.get_param(
            "config.endpoints", None, "Elasticsearch endpoint is missing"
        )
        self.kibana = self.get_param("config.kibana", None, None)
        self.index = self.get_param(
            "config.index", None, "Elasticsearch index is missing"
        )
    ...
    def run(self):
        Analyzer.run(self)
        try:
            for endpoint, key, user, password in zip(
                self.endpoints, self.keys, self.users, self.passwords
            ):
                if key:
                    es = Elasticsearch(
                        endpoint,
                ...
                if (
                    ...
                    and self.data_type != "url"
                    and self.data_type != "fqdn"
                ):
                    self.fields = list(
                        filter(lambda s: not ("url" in s or "domain" in s), self.fields)
                    )
                if self.kibana and self.dashboard:
                    ...
                    for field in self.fields[1:]:
                        info["query"] += (
                            "%22%20or%20" + field + "%20:%20%22" + self.data
                        )
                ...
                for index in self.index:
                    body = {
                        ...
                        "query": {
                            "multi_match": {"query": self.data, "fields": self.fields}
                        },
                    }
                    ...
                    for hit in res["hits"]["hits"]:
                        ...
                        if "@timestamp" in hit["_source"]:
                            if isinstance(hit["_source"]["@timestamp"], str):
                                ...
                                time = timestamp.astimezone().strftime(
                                    "%m/%d/%Y %I:%M %p"
                                )
                        ...
                        if "process" in hit["_source"]:
                            if "parent" in hit["_source"]["process"]:
                                if "name" in hit["_source"]["process"]["parent"]:
                                    process_parent_name = hit["_source"]["process"][
                                        "parent"
                                    ]["name"]
                        ...
                        if "source" in hit["_source"]:
                            ...
                            if "user" in hit["_source"]["source"]:
                                if "name" in hit["_source"]["source"]["user"]:
                                    source_user_name = hit["_source"]["source"]["user"][
                                        "name"
                                    ]
                        # event fields
                        ...
                        if "dns" in hit["_source"]:
                            if "question" in hit["_source"]["dns"]:
                                if "name" in hit["_source"]["dns"]["question"]:
                                    dns_question_name = hit["_source"]["dns"][
                                        "question"
                                    ]["name"]
                ...
                self.report({"hits": hits, "info": info, "total": total})
