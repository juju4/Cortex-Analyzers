    ...
    def check_file(self, **kwargs):
        ...
        try:
            if kwargs.get("filetype") in [
                ...
                kwargs.get("mimetype").startswith(
        ...
        for section in self.results:
            if section["submodule_section_header"] == "Olevba":
                ...
                if "VBA string" in type_list:
                    taxonomies.append(
                        self.build_taxonomy(level, namespace, predicate, "VBA string")
                    )
                    add_VBA = False
                if "Base64 String" in type_list:
                    taxonomies.append(
                        self.build_taxonomy(
                            level, namespace, predicate, "Base64 string"
                        )
                    )
                    add_VBA = False
                if "Hex String" in type_list:
                    taxonomies.append(
                        self.build_taxonomy(level, namespace, predicate, "Hex string")
                    )
                    add_VBA = False
                if "VBA" in type_list and add_VBA:
                    taxonomies.append(
                        self.build_taxonomy(level, namespace, predicate, "Macro found")
                    )
                if "None" in type_list:
                    taxonomies.append(
                        self.build_taxonomy("safe", namespace, predicate, "None")
                    )
            ...
            if section["submodule_section_header"] == "DDE Analysis":
                ...
                if section["submodule_section_content"].get("Info"):
                    level = "safe"
                    taxonomies.append(
                        self.build_taxonomy(level, namespace, predicate, "None")
                    )
                else:
                    level = "suspicious"
                    taxonomies.append(
                        self.build_taxonomy(level, namespace, predicate, "URL found")
                    )
