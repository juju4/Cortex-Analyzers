...
class StaxxAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.auth_url = self.get_param(
            "config.auth_url", None, "Missing URL for Staxx API auth"
        )
        self.query_url = self.get_param(
            "config.query_url", None, "Missing URL for Staxx API query"
        )
        self.username = self.get_param(
            "config.username", None, "Missing username for Staxx API"
        )
        self.password = self.get_param(
            "config.password", None, "Missing password for Staxx API"
        )
    ...
    def staxx_query(self, data):
        ...
        r = requests.post(
            self.auth_url, json=auth_data, headers=headers, verify=self.ssl
        )
        r.raise_for_status()  # Raise exception on HTTP errors
        token_id = r.json()["token_id"]
        pull_data = {"token": token_id, "query": data, "type": "json"}
        p = requests.post(
            self.query_url, json=pull_data, headers=headers, verify=self.ssl
        )
