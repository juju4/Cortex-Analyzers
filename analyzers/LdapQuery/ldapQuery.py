...
class LdapQuery(Analyzer):
    def __init__(self):
        ...
        if self.data_type == "mail":
            self.search_fields = self.get_param(
                "config.mail_search_fields", None, "mail_search_fields is missing"
            )
            self.filters = self.get_param("config.mail_search_filter", None)
        else:
            self.search_fields = self.get_param(
                "config.uid_search_fields", None, "uid_search_fields is missing"
            )
        ...
        self.attributes_to_extract, self.attributes_to_extract_types = (
            self.get_attribute_mapping("config.attributes_to_extract")
        )
        # Get attributes to export as tags
        self.attributes_to_tags, self.attributes_to_tags_prefix = (
            self.get_attribute_mapping("config.attributes_to_tags")
        )
        # Get attributes to export as custom fields
        self.attributes_to_custom_fields, self.attributes_to_custom_fields_prefix = (
            self.get_attribute_mapping("config.attributes_to_custom_fields")
        )
    ...
    def get_attribute_mapping(self, config_param):
        ...
        for item in self.get_param(config_param, []):
            if isinstance(item, str):
                ...
                if (
                    split_item[-1] == ""
                ):  # To handle "mail:" the same way as "mail:mail" or "mail"
    ...
    def summary(self, raw):
        ...
        if raw.get("filtered", None):
            taxonomies.append(
                self.build_taxonomy("suspicious", namespace, predicate, "filtered")
            )
            return {"taxonomies": taxonomies}
        # Summary for empty results
        if raw.get("results", None) == []:
            taxonomies.append(
                self.build_taxonomy("malicious", namespace, predicate, "no_result")
            )
            return {"taxonomies": taxonomies}
        ...
        if self.filters:
            ...
            for pattern in self.filters:
                if (
                    ...
                    and search(pattern, data_to_check) is not None
                ):
                ...
                for response in responses:
                    ...
                    if dict_response:
                        for att in dict_response.keys():
                            ...
                            if attribute == "" or attribute == [] or attribute is None:
                                continue
        ...
        for user in raw.get("results", []):
            ...
            for att in user.keys():
                if att in self.attributes_to_tags:
                    ...
                    tags.append(
                        self.attributes_to_tags_prefix[index]
                        + ":"
                        + str(user.get(att, None))
                    )
            ...
            for att in user.keys():
                ...
                if att in self.attributes_to_extract:
                    ...
                    artifacts.append(
                        self.build_artifact(
                            self.attributes_to_extract_types[index],
                            user.get(att, None),
                            tags=tags,
                        )
                    )
    ...
    def operations(self, raw):
        ...
        if raw.get("filtered", None):
            operations.append(
                self.build_operation("AddTagToArtifact", tag="ldap_filtered")
            )
            return operations
        ...
        for user in raw.get("results", []):
            if i > 0:
                index_prefix = (
                    "(" + str(i) + ")"
                )  # Prefix used in case multiple entries are returned
            for att in user.keys():
                if att in self.attributes_to_tags:  # Add tags
                    ...
                    operations.append(
                        self.build_operation(
                            "AddTagToArtifact",
                            tag=self.attributes_to_tags_prefix[index]
                            + index_prefix
                            + ":"
                            + str(user.get(att, None)),
                        )
                ...
                if att in self.attributes_to_custom_fields:  # Add Custom Fileds
                    ...
                    operations.append(
                        self.build_operation(
                            "AddCustomFields",
                            name=self.attributes_to_custom_fields_prefix[index]
                            + ":"
                            + str(user.get(att, None)),
                            value=str(user.get(att, None)),
                            tpe="string",
                        )
                    )
            i += 1
        ...
        if len(raw.get("results", [])) > 0:
            ...
            operations.append(
                self.build_operation("AddTagToArtifact", tag="ldap_no_result")
            )
