...
class TriageAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.app_secret = self.get_param(
            "config.api_key", None, "Triage API key is missing"
        )
        ...
        self.url = self.get_param(
            "config.api_url", "https://private.tria.ge/api", "Triage API URL is missing"
        )
    ...
    def summary(self, raw):
        ...
        taxonomies.append(self.build_taxonomy(verdict, namespace, "Score", value))
        ...
        if zipfile.is_zipfile(filepath):
            ...
            for zinfo in zf.infolist():
                ...
                if is_encrypted:
                    ...
                    submit = connect.submit_sample_file(
                        filename, sample, password=password
                    )
                else:
                    sample = open(filepath, "rb")
    ...
    def run(self):
        ...
        if self.data_type == "ip" or self.data_type == "url":
            ...
            if ":" in data:
                ...
                self.report({"result": result, "url": base_url})
            else:
