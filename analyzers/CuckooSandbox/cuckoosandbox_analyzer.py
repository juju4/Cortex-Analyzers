...
class CuckooSandboxAnalyzer(Analyzer):
    ...
    def summary(self, raw):
        ...
        taxonomies.append(
            self.build_taxonomy(
                level, namespace, predicate, "{}".format(result["malscore"])
            )
        )
        taxonomies.append(
            self.build_taxonomy(
                level, namespace, "Malfamily", "{}".format(result["malfamily"])
            )
        )
        ...
        try:
            ...
            while not finished and tries <= 15:  # wait max 15 mins
                ...
                response = requests.get(
                    self.url + "tasks/view/" + str(task_id),
                    headers=headers,
                    verify=self.verify_ssl,
                )
            ...
            # Download the report
            response = requests.get(
                self.url + "tasks/report/" + str(task_id) + "/json",
                headers=headers,
                verify=self.verify_ssl,
            )
            ...
            if (
                "suricata" in resp_json.keys()
                and "alerts" in resp_json["suricata"].keys()
            ):
                if any("dstport" in x for x in resp_json["suricata"]["alerts"]):
                    suri_alerts = [
                        (x["signature"], x["dstip"], x["dstport"], x["severity"])
                        for x in resp_json["suricata"]["alerts"]
                        if "dstport" in x.keys()
                    ]
                elif any("dst_port" in x for x in resp_json["suricata"]["alerts"]):
                    suri_alerts = [
                        (x["signature"], x["dst_ip"], x["dst_port"], x["severity"])
                        for x in resp_json["suricata"]["alerts"]
                    ]
                else:
                    suri_alerts = []
            ...
            if "snort" in resp_json.keys() and "alerts" in resp_json["snort"].keys():
                if any("dstport" in x for x in resp_json["snort"]["alerts"]):
                    snort_alerts = [
                        (x["message"], x["dstip"], x["dstport"], x["priority"])
                        for x in resp_json["snort"]["alerts"]
                    ]
                elif any("dst_port" in x for x in resp_json["snort"]["alerts"]):
                    snort_alerts = [
                        (x["message"], x["dst_ip"], x["dst_port"], x["priority"])
                        for x in resp_json["snort"]["alerts"]
                    ]
                else:
                    snort_alerts = []
            ...
            try:
                domains = (
                    [(x["ip"], x["domain"]) for x in resp_json["network"]["domains"]]
                    if "domains" in resp_json["network"].keys()
                    else None
                )
            except TypeError as e:
                domains = (
                    [x for x in resp_json["network"]["domains"]]
                    if "domains" in resp_json["network"].keys()
                    else []
                )
            uri = (
                [(x["uri"]) for x in resp_json["network"]["http"]]
                if "http" in resp_json["network"].keys()
                else []
            )
                ...
                self.report(
                    {
                        ...
                        "file_type": "url",
                        "yara": resp_json["target"]["url"]
                        if "target" in resp_json.keys()
                        and "url" in resp_json["target"].keys()
                        else "-",
                    }
                )
            else:
                self.report(
                    {
                        ...
                        "malfamily": resp_json.get("malfamily", None),
                        "file_type": "".join(
                            [x for x in resp_json["target"]["file"]["type"]]
                        ),
                        "yara": [
