...
class WOTAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.WOT_id = self.get_param("config.user", None, "Missing WOT API user")
        self.WOT_key = self.get_param("config.key", None, "Missing WOT API key")

    def wot_checkurl(self, data):
        url = "http://scorecard.api.mywot.com/v3/targets?t={}".format(data)
        headers = {"x-user-id": self.WOT_id, "x-api-key": self.WOT_key}
    ...
    def summary(self, raw):
        ...
        taxonomies.append(
            self.build_taxonomy(level, "WOT", "Category", "{}".format(value))
        )
