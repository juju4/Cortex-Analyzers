        ...
        try:
            ...
            if "error" in scanid_result.keys():
                if scanid_result["error"] > 0:
                    error_text = (
                        "Scan launch failed. Error code "
                        + str(scanid_result["Error"])
                        + " : "
                        + scanid_result["text"]
                    )
                ...
                error_text = "Scan launch failed. API said : {say}".format(
                    say=str(scanid_result)
                )
                raise ScanypheError(error_text)
        except Exception as e:
            self.unexpectedError(e)
        ...
        try:
            ...
            for x in range(intervals):
                ...
                if results["error"] == 0:
                    ...
                    error_text = (
                        "Scan results fetch failed after "
                        + str(waited)
                        + " secs. Error code "
                        + str(results["error"])
                        + " : "
            ...
            if scan_finished == False:
                raise ScanypheError(
                    "Scan failed : no results were received from Scanyphe after {say} secs".format(
                        say=str(waited)
                    )
                )
