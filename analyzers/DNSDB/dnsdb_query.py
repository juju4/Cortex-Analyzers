...
DEFAULT_CONFIG_FILES = filter(
    os.path.isfile, ("/etc/dnsdb-query.conf", os.path.expanduser("~/.dnsdb-query.conf"))
)
...
def time_parse(s):
    ...
    m = re.match(
        r"^(?=\d)(?:(\d+)w)?(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s?)?$", s, re.I
    )
    if m:
        return -1 * (
...
def main():
    ...
    parser.add_option(
        "-c", "--config", dest="config", help="config file", action="append"
    )
    parser.add_option(
        "-r",
        "--rrset",
    ...
    parser.add_option(
        "-t", "--rrtype", dest="rrtype", type="string", help="rrset or rdata rrtype"
    )
    parser.add_option(
        "-b", "--bailiwick", dest="bailiwick", type="string", help="rrset bailiwick"
    )
    ...
    if options.rrset:
        ...
        results = client.query_rdata_name(
            *qargs, before=options.before, after=options.after
        )
        fmt_func = rdata_to_text
    elif options.rdata_ip:
        results = client.query_rdata_ip(
            options.rdata_ip, before=options.before, after=options.after
        )
        fmt_func = rdata_to_text
    else:
        parser.print_help()
        ...
        if options.sort:
            ...
            if len(results) > 0:
                if not options.sort in results[0]:
                    ...
                    sort_keys.sort()
                    sys.stderr.write(
                        'dnsdb_query: invalid sort key "%s". valid sort keys are %s\n'
