...
class Splunk(Analyzer):
    def __init__(self):
        ...
        self.PORT_GUI = self.getParam(
            "config.port_gui", None, "GUI port parameter is missing"
        )
        ...
        self.APP = self.getParam(
            "config.application", None, "Application parameter is missing"
        )
        self.SAVEDSEARCHES = self.getParam(
            "config.saved_searches",
            None,
            "At least one Splunk savedsearch name is required",
        )
    ...
    def SplunkSearch(self, **kwargs_savedsearch):
        ...
        for saved_search in saved_searches:
            ...
            job = self.service.saved_searches[saved_search].dispatch(
                **kwargs_savedsearch
            )
            jobs[saved_search] = {
                "job": job,
        ...
        while jobs_running != 0:
            ...
            for saved_search in jobs:
                ...
                if job.is_done():
                    try:
                        jobs[saved_search]["results"] = results.ResultsReader(
                            job.results(count=self.MAX_COUNT)
                        )
                        ...
                        jobs[saved_search]["searchEarliestTime"] = (
                            datetime.utcfromtimestamp(
                                round(float(job["searchEarliestTime"]))
                            ).strftime("%c")
                        )
                        jobs[saved_search]["searchLatestTime"] = (
                            datetime.utcfromtimestamp(
                                round(float(job["searchLatestTime"]))
                            ).strftime("%c")
                        )
            ...
            try:
                ...
                if (
                    ...
                    jobResult["levels"] = {
                        "info": fieldLevelInfo,
                        "safe": fieldLevelSafe,
                        "suspicious": fieldLevelSuspicious,
                        "malicious": fieldLevelMalicious,
                    }
                jobResult["results"] = dataResults

            except Exception as e:
                jobResult["error"] = (
                    "Parsing results error for this search, special character ? : "
                    + str(e)
                )
                ...
                if jobResult["resultCount"] > self.MAX_COUNT:
                    jobResult["note"] = (
                        "Only the first {} results were recovered over {} to avoid any trouble on TheHive/Cortex. This parameter (max_count) can be changed in the analyzer configuration.".format(
                            self.MAX_COUNT, jobResult["resultCount"]
                        )
                    )
