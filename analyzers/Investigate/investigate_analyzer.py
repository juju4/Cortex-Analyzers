...
class InvestigateAnalyzer(Analyzer):
    ...
    def summary(self, raw):
        ...
        if self.service == "categorization":
            ...
            taxonomies.append(
                self.build_taxonomy(
                    level, namespace, predicate, "{}".format(self.STATUS_MAP[status])
                )
            )
            ...
            display_str = (
                ", ".join(security_categories) if security_categories else "None"
            )
            taxonomies.append(
                self.build_taxonomy(
                    level, namespace, predicate, "{}".format(display_str)
                )
            )
            ...
            display_str = (
                ", ".join(content_categories) if content_categories else "None"
            )
            taxonomies.append(
                self.build_taxonomy(
                    level, namespace, predicate, "{}".format(display_str)
