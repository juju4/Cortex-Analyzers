...
class DomainMailSPFDMARC(Analyzer):
    ...
    def summary(self, raw):
        ...
        if "error" in raw["DomainMailSPFDMARC"]["dmarc"]:
            if "error" in raw["DomainMailSPFDMARC"]["spf"]:
                ...
                taxonomies.append(
                    self.build_taxonomy("suspicious", namespace, "DMARC", "no")
                )
        else:
            if "error" in raw["DomainMailSPFDMARC"]["spf"]:
                taxonomies.append(
                    self.build_taxonomy("suspicious", namespace, "SPF", "no")
                )
                taxonomies.append(
                    self.build_taxonomy("safe", namespace, "DMARC", "yes")
                )
            else:
                taxonomies.append(self.build_taxonomy("safe", namespace, "SPF", "yes"))
                taxonomies.append(
    ...
    def run(self):
        if self.data_type == "domain" or self.data_type == "fqdn":
            ...
            self.error(
                "Data type not supported. Please use this analyzer with data types domain or fqdn."
            )
