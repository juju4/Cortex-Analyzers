...
class PatrowlAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.service = self.get_param(
            "config.service", None, "Patrowl service is missing"
        )
        self.url = self.get_param("config.url", None, "Patrowl URL is missing").rstrip(
            "/"
        )
        self.api_key = self.get_param(
            "config.api_key", None, "Patrowl API Key is missing"
        )
    ...
    def summary(self, raw):
        ...
        if self.service == "getreport":
            if "risk_level" in raw and raw["risk_level"]:
                ...
                taxonomies.append(
                    self.build_taxonomy(level, namespace, "Grade", risk_level["grade"])
                )
