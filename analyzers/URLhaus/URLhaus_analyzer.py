...
class URLhausAnalyzer(Analyzer):
    ...
    def run(self):
        ...
        results.update({"data_type": self.data_type})
        self.report(results)
    ...
    def summary(self, raw):
        ...
        if (
            ...
            or raw["query_status"] == "ok"
            and raw["md5_hash"] == None
            and raw["sha256_hash"] == None
        ):
            taxonomies.append(
                self.build_taxonomy("info", namespace, "Search", "No results")
            )
        else:
            if self.data_type == "url":
                taxonomies.append(
                    self.build_taxonomy("malicious", namespace, "Threat", raw["threat"])
                )
            elif self.data_type in ["domain", "fqdn", "ip"]:
                threat_types = []
                for url in raw["urls"]:
                ...
                taxonomies.append(
                    self.build_taxonomy(
                        ...
                        "Signature",
                        raw["signature"]
                        if raw["signature"] and raw["signature"] != "null"
                        else "Unknown",
                    )
                )
