...
class StamusNetworksAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.api_key = self.get_param(
            "config.key", None, "Scirius Security Platform api key is missing"
        )
        self.base_url = self.get_param(
            "config.url", None, "Scirius Security Platform url is missing"
        )
        self.base_url = self.base_url.rstrip("/ ")
        self.ssl_verify = self.get_param(
            "config.ssl_verify",
            None,
            "Scirius Security Platform TLS verification info is missing",
        )
    ...
    def artifacts(self, raw):
        ...
        for host in hostnames:
            ...
            artifacts.append(self.build_artifact("fqdn", host["host"], tags=tags))
        net_info = raw["host_id"].get("net_info", [])
        if len(net_info) > 0:
            net_info = sorted(net_info, key=lambda k: k["last_seen"], reverse=True)[0][
                "agg"
            ]
            tags = ["network-info"]
            artifacts.append(self.build_artifact("other", net_info, tags=tags))
        return artifacts
    ...
    def summary(self, raw):
        ...
        if value:
            taxonomies.append(
                self.build_taxonomy("info", namespace, "tls-agents", value)
            )
        value = raw["host_id"].get("http.user_agent_count")
        if value:
            taxonomies.append(
                self.build_taxonomy("info", namespace, "http-agents", value)
            )
        ...
        try:
            if self.data_type == "ip":
                ...
                resp = self.session.get(
                    url, verify=self.ssl_verify, proxies=self.proxies
                )
