...
class GoogleAPI_Vision(Analyzer):
    ...
    def get_artifacts(self, google_results: dict) -> list:
        return [
            item["url"]
            for item in google_results["responses"][0]["webDetection"][
    ...
    def run(self):
        ...
        api_key = self.get_param(
            "config.api_key", None, "You need an API key for Google Vision API"
        )
        https_proxy = {"https": self.get_param("config.proxy_https")}
    ...
    def summary(self, raw):
        number_of_image_found = self.build_taxonomy(
            "info",
            "GoogleVisionAPI",
            "pagesWithMatchingImages",
            str(
                len(
                    raw["api_full_report"]["responses"][0]["webDetection"][
                        "pagesWithMatchingImages"
                    ]
                )
            ),
        )
        number_of_look_alike = self.build_taxonomy(
            "info",
            "GoogleVisionAPI",
