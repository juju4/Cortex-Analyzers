...
class CIRCLPassiveDNSAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.pdns = pypdns.PyPDNS(
            basic_auth=(
                self.get_param("config.user", None, "No passiveDNS username given."),
                self.get_param(
                    "config.password", None, "No passiveDNS password given."
                ),
            )
        )
    ...
    def run(self):
        ...
        if self.data_type == "url":
            ...
            if "/" in query:
                self.error(
                    "'/' found in the supplied domain. use the URL datatype instead"
                )
