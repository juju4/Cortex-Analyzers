...
class PayloadSecurityAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.apikey = self.get_param(
            "config.key", None, "PayloadSecurity apikey is missing"
        )
        self.secret = self.get_param(
            "config.secret", None, "PayloadSecurity secret is missing"
        )
        self.environmentid = self.get_param(
            "config.environmentId", None, "PayloadSecurity environmentId is missing"
        )
    ...
    def run(self):
        ...
        try:
            ...
            # URL
                ...
                data = {
                    "environmentId": self.environmentid,
                    "analyzeurl": self.get_data(),
                    "comment": "Submitted by Cortex",
                }
                response = requests.post(
                    self.url.strip("/") + "/api/submiturl",
                    data=data,
                    headers=user_agent,
                    verify=self.verify,
                    auth=HTTPBasicAuth(self.apikey, self.secret),
            ...
            while not finished and tries <= self.timeout:
                ...
                if (
                    ...
                    and data["response"]["state"] == "SUCCESS"
                ):
                    finished = True
                tries += 1
            if not finished:
            ...
            if response.status_code == 200:
                ...
                if (
                    ...
                    and data["response"].get("screenshots")
                ):
