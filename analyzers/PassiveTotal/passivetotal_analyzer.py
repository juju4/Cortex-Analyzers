...
class PassiveTotalAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.service = self.get_param(
            "config.service", None, "PassiveTotal service is missing"
        )
        self.username = self.get_param(
            "config.username", None, "PassiveTotal username is missing"
        )
        self.api_key = self.get_param(
            "config.key", None, "PassiveTotal API key is missing"
        )
    ...
    def summary(self, raw):
        ...
        # whois details service
            ...
            if (
                "registrant" in raw
                and "organization" in raw["registrant"]
                and raw["registrant"]["organization"]
            ):
        ...
        try:
            ...
            # ssl certificate history service
                ...
                if (
                    ...
                    and result["results"][0]["ipAddresses"] == "N/A"
                ):
                    print("ok")
                    self.report({"results": []})
                else:
            ...
            # host pairs service
                ...
                result = host_attr_request.get_host_pairs(
                    query=data, direction="parents"
                )
