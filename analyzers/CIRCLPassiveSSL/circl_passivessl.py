...
class CIRCLPassiveSSLAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.user = self.get_param(
            "config.user", None, "PassiveSSL username is missing!"
        )
        self.password = self.get_param(
            "config.password", None, "PassiveSSL password is missing!"
        )
    ...
    def query_ip(self, ip):
        ...
        newresult = {"ip": ip, "certificates": []}
        for cert in certificates:
            if cert not in subjects:
                continue
            newresult["certificates"].append(
                {"fingerprint": cert, "subject": subjects.get(cert).get("values")[0]}
            )
        return newresult
    ...
    def artifacts(self, raw):
        ...
        if "query" in raw:
            for ip in raw.get("query").get("seen"):
                artifacts.append(self.build_artifact("ip", str(ip)))
        return artifacts
    ...
    def run(self):
        if self.data_type == "certificate_hash" or self.data_type == "hash":
            ...
            if len(data) != 40:
                self.error(
                    "CIRCL Passive SSL expects a sha1 hash, given hash has more or less than 40 characters."
                )
