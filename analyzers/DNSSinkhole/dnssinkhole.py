...
class DNSSinkholeAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.resolver.nameservers = [
            self.get_param(
                "config.ip", None, "Bind IP server needed for querying Sinkhole."
            )
        ]
    ...
    def run(self):
        self.report({"is_sinkhole": self.query()})
    ...
    def summary(self, raw):
        ...
        if raw.get("is_sinkhole"):
            taxonomies.append(
                self.build_taxonomy("malicious", "DNSSinkhole", "IsSinkhole", "True")
            )
        else:
            taxonomies.append(
                self.build_taxonomy("info", "DNSSinkhole", "IsSinkhole", "False")
            )
