...
class IPAPI(Analyzer):
    ...
    def run(self):
        ...
        if self.data_type in ("ip", "domain"):
            data = self.get_data()
            base_url = (
                "https://pro.ip-api.com/json" if self.key else "http://ip-api.com/json"
            )
            ...
            params = (
                {"key": self.key, "fields": fields} if self.key else {"fields": fields}
            )

            try:
                with requests.Session() as session:
                    response = session.get(
                        f"{base_url}/{data}", params=params, timeout=10
                    )
