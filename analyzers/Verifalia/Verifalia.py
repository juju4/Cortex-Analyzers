...
class Verifalia(Analyzer):
    def __init__(self):
        ...
        self.login_key = self.get_param(
            "config.login", None, "Missing username for Verifalia"
        )
        self.password_key = self.get_param(
            "config.password", None, "Missing password for Verifalia"
        )
            ...
            try:
                ...
                with requests.Session() as s:
                    ...
                    response = s.post(
                        ...
                        data=data,
                        auth=(
                            "{}".format(self.login_key),
                            "{}".format(self.password_key),
                        ),
                    )
                    id_of_case = response.json()["overview"]["id"]
                    time.sleep(10)
                    response_details = s.get(
                        "https://api.verifalia.com/v2.2/email-validations/{}".format(
                            id_of_case
                        ),
                        auth=(
                            "{}".format(self.login_key),
                ...
                if response_details.status_code == 200:
                    ...
                    self.error(
                        "Failed to query Verifalia details. Status_code {}".format(
