...
class ShodanAnalyzer(Analyzer):
    ...
    def execute_shodan_service(self, data):
        if self.service in ["host", "host_history"]:
            results = {
                "host": self.shodan_client.host(
                    data, history=True if self.service == "host_history" else False
                )
            }
            return results
    ...
    def summary(self, raw):
        ...
        if self.service in ["host", "host_history"]:
            ...
            if "org" in raw["host"]:
                taxonomies.append(
                    self.build_taxonomy(level, namespace, "Org", raw["host"]["org"])
                )
            if "asn" in raw["host"]:
                taxonomies.append(
                    self.build_taxonomy(level, namespace, "ASN", raw["host"]["asn"])
                )
            if "vulns" in raw["host"]:
                ...
                if totalcve < 3:
                    taxonomies.append(
                        self.build_taxonomy(
                            levelorange, namespace, "VULNS", raw["host"]["vulns"]
                        )
                    )
                else:
                    taxonomies.append(
                        self.build_taxonomy(levelorange, namespace, "VULNS", totalcve)
                    )
        elif self.service == "info_domain":
            ...
            if "all_domains" in raw["info_domain"]:
                ...
                taxonomies.append(
                    self.build_taxonomy(level, namespace, "Domains", value)
                )
            ...
            taxonomies.append(
                self.build_taxonomy(level, namespace, "DNS resolutions", value)
            )
        elif self.service == "reverse_dns":
            nb_domains = 0
            ...
            taxonomies.append(
                self.build_taxonomy(level, namespace, "Reverse DNS resolutions", value)
            )
        elif self.service == "search":
