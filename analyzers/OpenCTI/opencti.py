...
class OpenCTIAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.service = self.get_param("config.service", "search_exact", None)
        ...
        if len(names) != len(urls) or len(urls) != len(keys):
            self.error(
                "Config error: please add a name, an url and a key for each OpenCTI instance."
            )
        ...
        for opencti in self.openctis:
            ...
            if self.service == "search_exact":
                ...
                observables = [
                    ...
                    if observable["observable_value"] == data
                ]
