...
class RobtexAnalyzer(Analyzer):
    ...
    def query_ip(self):
        ...
        """
        return requests.get(
            "https://freeapi.robtex.com/ipquery/{}".format(self.get_data())
        ).json()
    ...
    def query_rpdns(self):
        ...
        results = requests.get(
            "https://freeapi.robtex.com/pdns/reverse/{}".format(self.get_data())
        ).text.split("\r\n")
        jsonresults = []
    ...
    def query_fpdns(self):
        ...
        """
        results = requests.get(
            "https://freeapi.robtex.com/pdns/forward/{}".format(self.get_data())
        ).text.split("\r\n")
        jsonresults = []
        for idx, r in enumerate(results):
            if len(r) > 0:
    ...
    def run(self):
        ...
        elif self.mode == "fpdnsquery" and self.get_param("dataType", None) in [
            "fqdn",
            "domain",
        ]:
