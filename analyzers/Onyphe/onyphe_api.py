...
class Onyphe:
    ...
    def _request(self, path: str, query_params: dict = {}):
        ...
        try:
            ...
            raise APIGeneralError(
                "Couldn't connect to ONYPHE API : {url}".format(url=url)
            )
        ...
        if response_data["error"] > 0:
            raise APIGeneralError(
                "Error {code} {text} : {url}".format(
                    code=response_data["error"], text=response_data["text"], url=url
                )
            )
    ...
    def search(self, data: str, datatype: str, category: str, filter: str):
        """Return data from specified category using Search API and the provided data as the OQL filter."""
        if datatype == "ip":
            url_path = "search/?q=category:{category}+ip:{ip}+{filter}".format(
                category=category, ip=data, filter=filter
            )
        elif datatype == "domain":
            url_path = "search/?q=category:{category}+domain:{domain}+{filter}".format(
                category=category, domain=data, filter=filter
            )
        elif datatype == "fqdn":
            url_path = (
                "search/?q=category:{category}+hostname:{hostname}+{filter}".format(
                    category=category, hostname=data, filter=filter
                )
            )
        elif datatype == "hash":
            url_path = "search/?q=category:{category}+fingerprint.sha256:{hash}+{filter}".format(
                category=category, hash=data, filter=filter
            )
        return self._request(path=url_path)
