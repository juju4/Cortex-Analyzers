...
class OnypheAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.onyphe_category = self.get_param(
            "config.category", "datascan"
        )  # only used for Search service
        self.time_filter = self.get_param("config.time_filter", "-since:1M")
        self.auto_import = self.get_param("config.auto_import", False)
        self.verbose_taxonomies = self.get_param(
            "config.verbose_taxonomies", False
        )  # Only used for Summary service
        self.polling_interval = self.get_param("config.polling_interval", 60)
        self.base_url = self.get_param(
            "config.base_url", "https://www.onyphe.io"
        ) + self.get_param(
            "config.base_uri", "/api/v2/"
        )  # Trailing / is needed for urljoin
        self.return_other_artifacts = self.get_param(
            "config.return_other_artifacts", False
        )
    ...
    def summary(self, raw):
        ...
        if self.service != "summary":
            ...
            for odoc in raw["results"]:
                if (
                    odoc["@category"] == "ctiscan"
                    and "http" in odoc
                    and "vhost" in odoc["http"]
                ):
                    dedupkey = (
                        str(odoc["ip"]["dest"])
                        + ":"
                    ...
                    odoc["@category"] == "ctiscan" and odoc["app"]["transport"] == "tcp"
                ):
                    dedupkey = str(odoc["ip"]["dest"]) + ":" + str(odoc["tcp"]["dest"])
                elif (
                    odoc["@category"] == "ctiscan" and odoc["app"]["transport"] == "udp"
                ):
            ...
            if len(reportlist) > 0:
                taxonomies.append(
                    self.build_taxonomy(
                        ...
                        namespace,
                        "Services",
                        "{} unique services (ip:port:vhost) found".format(
                            len(reportlist)
                        ),
                    )
                )
            ...
            try:
                for odoc in raw["results"]:
                    if (
                        ...
                        dedupkey = (
                            str(odoc["ip"]["dest"]) + ":" + str(odoc["tcp"]["dest"])
                        )
                    elif odoc["@category"] == "ctiscan" and "udp" in odoc:
                        dedupkey = (
                            str(odoc["ip"]["dest"]) + ":" + str(odoc["udp"]["dest"])
                        )
                    ...
                    if newasset:
                        ...
                        otags = []
                        if odoc["@category"] == "riskscan" or (
                            odoc["@category"] == "ctiscan" and "tag" in odoc
                        ):
                            for ta in odoc["tag"]:
                                if ta.split("::")[0] == "risk":
                                    otags.append(str(ta))
                        ...
                        if "transport" in odoc and "port" in odoc:
                            otags.append(
                                str(odoc["transport"]) + "/" + str(odoc["port"])
                            )
                        elif "tcp" in odoc:
                        ...
                        if self.return_other_artifacts:
                            ...
                            if "forward" in odoc:
                                ...
                                thisartifact = "fqdn#" + str(
                                    odoc["cert"]["hostname"][0]
                                )
                        else:
                            if "forward" in odoc:
                                thisartifact = "fqdn#" + str(odoc["forward"])
                ...
                for odoc in raw["results"]:
                    ...
                    if self.data_type == "fqdn":
                        if "forward" in odoc and odoc["forward"] == data:
                            ...
                            "http" in odoc
                            and "vhost" in odoc["http"]
                            and odoc["http"]["vhost"] == data
                        ):
                            matchdata = True
                        elif (
                            "dns" in odoc
                            and "hostname" in odoc["dns"]
                            and data in odoc["dns"]["hostname"]
                        ):
                            matchdata = True
                        elif (
                            "cert" in odoc
                            and "hostname" in odoc["cert"]
                            and data in odoc["cert"]["hostname"]
                        ):
                            matchdata = True
    ...
    def run(self):
        ...
        try:
            ...
            if self.service == "search":
                results = self.onyphe_client.search(
                    data, self.data_type, self.onyphe_category, self.time_filter
                )
                results["category"] = self.onyphe_category
                results["total_category"] = len(results["results"])

            elif self.service == "asm":
                self.onyphe_category = (
                    "riskscan"  # ASM service so force category to riskscan
                )
                self.fields_filter = self.get_param(
                    "config.fields_filter",
                ...
                asmfilter = (
                    self.time_filter + "+-fields:" + self.fields_filter
                )  # Fields filter is faster and saves space in The Hive database.
                results = self.onyphe_client.search(
                    data,
                    self.data_type,
                    self.onyphe_category,
                    asmfilter.replace(",", "%2C"),
                )
                ...
                results = self.onyphe_client.search(
                    data, self.data_type, self.onyphe_category, vulnfilter
                )
                results["category"] = self.onyphe_category
                results["total_category"] = len(results["results"])
                ...
                if self.data_type == "ip":
                    ...
                    ctifilter += (
                        "?dns.hostname:{hostname} ?cert.hostname:{hostname} ".format(
                            hostname=data
                        )
                    )
                elif self.data_type == "domain":
                    ctifilter += "?dns.domain:{domain} ?cert.domain:{domain} ".format(
                        domain=data
                    )
                    ...
                    try:
                        ...
                        if splitsize > 2 and splitsize < 10:
                            ...
                            if len(test) < 4:
                                raise OtherError(
                                    "Unable to parse observable {other} as type other".format(
                                        other=data
                                    )
                                )
                        else:
                        ...
                        ctifilter += "ip.dest:{ip} tcp.dest:{port} ".format(
                            ip=ip, port=port
                        )
                    except:
                        raise OtherError(
                ...
                oql = (
                    "category:{category} ".format(category=self.onyphe_category)
                    + ctifilter
                )
                ...
                results["totals_category"] = {
                    k: len([x for x in results["results"] if x["@category"] == k])
                    for k in [
                        "threatlist",
                        "threats",
                        "geoloc",
