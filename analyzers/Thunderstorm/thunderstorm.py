...
class ThunderstormAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.thunderstorm_server = self.get_param(
            "config.thunderstorm_server",
            None,
            "THOR Thunderstorm server has not been configured",
        )
        self.thunderstorm_port = self.get_param("config.thunderstorm_port", 8080)
        self.thunderstorm_source = self.get_param(
            "config.thunderstorm_source", "cortex-analyzer"
        )
        self.thunderstorm_ssl = self.get_param("config.thunderstorm_ssl", False)
        self.thunderstorm_verify_ssl = self.get_param(
            "config.thunderstorm_ssl_verify", False
        )
