...
class MalpediaAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        if hours > self.update_hours or len(timestamps) == 0:
            try:
                req = requests.get(
                    "{}/yara/after/2010-01-01?format=json".format(self.baseurl),
                ...
                if req.status_code == requests.codes.ok:
                    ...
                    for color, color_data in rules_json.items():
                        for rule_name, rule_text in color_data.items():
                            with io.open(
                                os.path.join(self.rulepaths, rule_name),
                                "w",
                                encoding="utf-8",
                            ) as f:
                                f.write(rule_text)
                    ...
                    self.error(
                        "Could not download new rules due tue HTTP {}: {}".format(
                            req.status_code, req.text
                        )
                    )
            except Exception as e:
                with io.open(
                    "%s" % os.path.join(self.rulepaths, "error.txt"), "w"
                ) as f:
                    f.write("Error: {}\n".format(e))
    ...
    def run(self):
        if self.data_type == "file":
            self.report(
                {"results": self.check(self.get_param("file", None, "No file given."))}
            )
        else:
