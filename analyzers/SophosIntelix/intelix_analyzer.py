...
class SophosIntelixAnalyzer(Analyzer):
    def __init__(self):
        ...
        self.clientSecret = self.get_param(
            "config.clientSecret", None, "Client Secret is Missing"
        )
    ...
    def summary(self, raw):
        ...
        if self.service == "get":
            if self.data_type in ("domain", "fqdn", "url"):
                ...
                predicate = "Score"
                value = "{} - {}".format(
                    self.ic.reputationScore, self.ic.classification
                )
            ...
            if self.ic.report.get("score") <= 19:
                ...
                self.ic.report.get("score") > 69 and self.ic.report.get("score") <= 100
            ):
                level = "safe"
            else:
                level = "info"
