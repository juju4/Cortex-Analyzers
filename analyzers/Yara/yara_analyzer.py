...
class YaraAnalyzer(Analyzer):
    def download_rules_from_github_url(self, url, token, limit=None):
        ...
        if response.status_code != 200:
            self.error(
                f"Error accessing repository tree: {response.status_code} - {response.text}"
            )
        ...
        for item in tree:
            if (
                ...
                and item["path"].endswith((".yar", ".yara", ".rule", ".rules"))
            ):
        ...
        for file_path in rule_files:
            ...
            if file_response.status_code != 200:
                self.error(
                    f"Error downloading file {file_path}: {file_response.status_code}"
                )
    ...
    def __init__(self):
        ...
        # Get local rules
        self.rulepaths = self.get_param(
            "config.rules", [], "No paths for rules provided."
        )
        ...
        for rulepath in self.rulepaths:
            if os.path.isfile(rulepath) and rulepath.endswith(
                (".yar", ".yara", ".rule", ".rules")
            ):
                rule_files.append(rulepath)
            elif os.path.isdir(rulepath):
                local_files = [
                    os.path.join(rulepath, f)
        ...
        if self.github_urls and self.github_token:
            for url in self.github_urls:
                if remaining is not None and remaining <= 0:
                    break
                github_files = self.download_rules_from_github_url(
                    url, self.github_token, limit=remaining
                )
    ...
    def check(self, file_path):
        ...
        for idx, rule_obj in enumerate(self.ruleset):
            try:
                ...
                self.error(
                    f"Error matching file '{file_path}' with ruleset from {rule_obj['source']} (index {idx}): {str(e)}"
                )
                continue
            for match in matches:
                try:
                            ...
                            decoded_strings.append(
                                {
                                    ...
                                    "hex_decoded": decoded_hex
                                    if decoded_hex
                                    else "N/A",
                                    "rot13_decoded": decoded_rot13
                                    if decoded_rot13
                                    else "N/A",
                                    "url_decoded": decoded_url
                                    if decoded_url
                                    else "N/A",
                                    "unicode_decoded": decoded_unicode
                                    if decoded_unicode
                                    else "N/A",
                                    "html_decoded": decoded_html
                                    if decoded_html
                                    else "N/A",
                                    "xor_decoded": decoded_xor
                                    if decoded_xor
                                    else "N/A",
                                }
                            )
                    ...
                    self.error(
                        f"Error processing match from rule '{match.rule}' in file {rule_obj['source']}: {str(e)}"
                    )
        return results
    ...
    def summary(self, raw):
        ...
        nb_of_rules = sum(
            len(rule_obj.get("rule_names", [])) for rule_obj in self.ruleset
        )
