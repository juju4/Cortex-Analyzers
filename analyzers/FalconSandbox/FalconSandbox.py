...
class FalconSandbox(Analyzer):
    def __init__(self):
        ...
        self.token = self.oauth.fetch_token(
            token_url=self.API_Base_Url + "/oauth2/token",
    ...
    def summary(self, raw):
        ...
        level = str(
            raw["resources"][0]["sandbox"][0]["verdict"]
        )  # no verdict, No Specific Threat, suspicious, malicious
    ...
    def run(self):
        try:
            ...
            json_response = self.rest_api_request(
            ...
            if json_response["errors"]:
                ...
                if not json_response["resources"]:
                    ...
                    json_response_submit = self.rest_api_request(
                    ...
                    if json_response_submit["errors"]:
                        ...
                        url = self.API_Base_Url + "/falconx/entities/submissions/v1"
                        headers = {"Content-Type": "application/json"}
                        payload = (
                            '{"sandbox": '
                            '[{"sha256": "'
                        ...
                        json_response_start_analysis = self.rest_api_request(
                            method="post", url=url, data=payload, headers=headers
                        )
                        if json_response_start_analysis["errors"]:
                            ...
                            while report_found == 0:
                                json_response = self.rest_api_request(
                                    method="get", url=url, data=payload, headers=headers
                                )
                                if json_response["errors"]:
                                    self.error(str(json_response["errors"]))
                                    return
                ...
                while analyzeinprogress == 1:
                    ...
                    payload = {}
                    headers = {"Content-Type": "application/json"}
                    json_response_report = self.rest_api_request(
                        method="get", url=url, data=payload, headers=headers
                    )
                    ...
                    if (
                        not json_response_report["resources"]
                        and json_response_report["meta"]["quota"]["in_progress"] != 0
                    ):
